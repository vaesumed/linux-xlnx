...
static int chsc_subchannel_match_next_free(struct device *dev, void *data)
{
	...
	return sch->schib.pmcw.ena && !sch->schib.scsw.cmd.fctl;
}
...
static int chsc_examine_irb(struct chsc_request *request)
{
	...
	if (!request->irb.scsw.cmd.stctl & SCSW_STCTL_STATUS_PEND)
		return -EIO;
	backed_up = request->irb.scsw.cmd.cstat & SCHN_STAT_CHAIN_CHECK;
	request->irb.scsw.cmd.cstat &= ~SCHN_STAT_CHAIN_CHECK;
	if (request->irb.scsw.cmd.cstat == 0)
		return 0;
	if (!backed_up)
		return 0;
	if (request->irb.scsw.cmd.cstat & SCHN_STAT_PROG_CHECK)
		return -EIO;
	if (request->irb.scsw.cmd.cstat & SCHN_STAT_PROT_CHECK)
		return -EPERM;
	if (request->irb.scsw.cmd.cstat & SCHN_STAT_CHN_DATA_CHK)
		return -EAGAIN;
	if (request->irb.scsw.cmd.cstat & SCHN_STAT_CHN_CTRL_CHK)
		return -EAGAIN;
	return -EIO;
}
