...
MODULE_DESCRIPTION("Device Driver Module for Meilhaus ME-4000 boards version 1.0.5");
...
MODULE_PARM_DESC(me4000_ao_major_driver_no, "Major driver number for analog output (default 0)");

module_param(me4000_ai_major_driver_no, int, 0);
MODULE_PARM_DESC(me4000_ai_major_driver_no, "Major driver number for analog input (default 0)");

module_param(me4000_dio_major_driver_no, int, 0);
MODULE_PARM_DESC(me4000_dio_major_driver_no, "Major driver number digital I/O (default 0)");

module_param(me4000_cnt_major_driver_no, int, 0);
MODULE_PARM_DESC(me4000_cnt_major_driver_no, "Major driver number for counter (default 0)");

module_param(me4000_ext_int_major_driver_no, int, 0);
MODULE_PARM_DESC(me4000_ext_int_major_driver_no, "Major driver number for external interrupt (default 0)");
...
static int me4000_get_user_info(me4000_user_info_t *, me4000_info_t *board_info);
...
/*-----------------------------------------------------------------------------
  Analog output stuff
  ---------------------------------------------------------------------------*/
static ssize_t me4000_ao_write_sing(struct file *, const char *, size_t, loff_t *);
static ssize_t me4000_ao_write_wrap(struct file *, const char *, size_t, loff_t *);
static ssize_t me4000_ao_write_cont(struct file *, const char *, size_t, loff_t *);

static int me4000_ao_ioctl_sing(struct inode *, struct file *, unsigned int, unsigned long);
static int me4000_ao_ioctl_wrap(struct inode *, struct file *, unsigned int, unsigned long);
static int me4000_ao_ioctl_cont(struct inode *, struct file *, unsigned int, unsigned long);
...
static int me4000_ao_simultaneous_update(me4000_ao_channel_list_t *channels, me4000_ao_context_t *ao_context);
...
static int me4000_ao_ex_trig_timeout(unsigned long *arg, me4000_ao_context_t *ao_context);
static int me4000_ao_get_free_buffer(unsigned long * arg, me4000_ao_context_t *ao_context);
...
static int me4000_ai_ioctl_sing(struct inode *, struct file *, unsigned int, unsigned long);

static ssize_t me4000_ai_read(struct file *, char *, size_t, loff_t *);
static int me4000_ai_ioctl_sw(struct inode *, struct file *, unsigned int, unsigned long);
static unsigned int me4000_ai_poll(struct file *, poll_table *);
static int me4000_ai_fasync(int fd, struct file *file_p, int mode);

static int me4000_ai_ioctl_ext(struct inode *, struct file *, unsigned int, unsigned long);
...
static int me4000_ai_ex_trig_setup(me4000_ai_trigger_t *, me4000_ai_context_t *);
static int me4000_ai_sc_setup(me4000_ai_sc_t *arg, me4000_ai_context_t *ai_context);
...
static int me4000_ai_get_count_buffer(unsigned long *arg, me4000_ai_context_t *ai_context);


/*-----------------------------------------------------------------------------
  EEPROM stuff
  ---------------------------------------------------------------------------*/
static int me4000_eeprom_read(me4000_eeprom_t *arg, me4000_ai_context_t *ai_context);
static int me4000_eeprom_write(me4000_eeprom_t *arg, me4000_ai_context_t *ai_context);
static unsigned short eeprom_read_cmd(me4000_ai_context_t *ai_context, unsigned long cmd, int length);
static int eeprom_write_cmd(me4000_ai_context_t *ai_context, unsigned long cmd, int length);


/*-----------------------------------------------------------------------------
  Digital I/O stuff
  ---------------------------------------------------------------------------*/
static int me4000_dio_ioctl(struct inode *, struct file *, unsigned int, unsigned long);
...
/*-----------------------------------------------------------------------------
  Counter stuff
  ---------------------------------------------------------------------------*/
static int me4000_cnt_ioctl(struct inode *, struct file *, unsigned int, unsigned long);
...
/*-----------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------*/
static int me4000_ext_int_ioctl(struct inode *, struct file *, unsigned int, unsigned long);
...
int __init me4000_init_module(void){
	...
	result = register_chrdev(me4000_ao_major_driver_no, ME4000_AO_NAME, &me4000_ao_fops_sing);
	if (result < 0){
	...
	result = register_chrdev(me4000_ai_major_driver_no, ME4000_AI_NAME, &me4000_ai_fops_sing);
	if (result < 0){
	...
	result = register_chrdev(me4000_dio_major_driver_no, ME4000_DIO_NAME, &me4000_dio_fops);
	if (result < 0){
		printk(KERN_ERR"ME4000:init_module():Can't get DIO major no\n");
		goto INIT_ERROR_4;
	}
	else{
	...
	result = register_chrdev(me4000_cnt_major_driver_no, ME4000_CNT_NAME, &me4000_cnt_fops);
	if (result < 0){
		printk(KERN_ERR"ME4000:init_module():Can't get CNT major no\n");
		goto INIT_ERROR_5;
	}
	else{
	...
	result = register_chrdev(me4000_ext_int_major_driver_no, ME4000_EXT_INT_NAME, &me4000_ext_int_fops);
	if (result < 0){
		printk(KERN_ERR"ME4000:init_module():Can't get major no for external interrupt\n");
		goto INIT_ERROR_6;
	}
	else{
		me4000_ext_int_major_driver_no = result;
	}
	PDEBUG("init_module():Major driver number for external interrupt = %ld\n", me4000_ext_int_major_driver_no);

	/* Create the /proc/me4000 entry */
	if(!create_proc_read_entry("me4000", 0, NULL, me4000_read_procmem, NULL)){
		result = -ENODEV;
		printk(KERN_ERR"ME4000:init_module():Can't create proc entry\n");
		goto INIT_ERROR_7;
	}
...
static void clear_board_info_list(void){
	...
	/* Clear context lists */
	for(board_p = me4000_board_info_list.next; board_p != &me4000_board_info_list; board_p = board_p->next){
		...
		while(!list_empty(&board_info->ao_context_list)){
			...
			if(ao_context->circ_buf.buf) kfree(ao_context->circ_buf.buf);
			list_del(dac_p);
			kfree(ao_context);
		}

		/* Clear analog input context */
		if(board_info->ai_context->circ_buf.buf) kfree(board_info->ai_context->circ_buf.buf);
...
static int get_registers(struct pci_dev *dev, me4000_info_t *board_info){
	...
	if(board_info->plx_regbase == 0){
		printk(KERN_ERR"ME4000:get_registers():PCI base address 1 is not available\n");
		return -ENODEV;
	}
	board_info->plx_regbase_size = pci_resource_len(dev, 1);

	PDEBUG("get_registers():PLX configuration registers at address 0x%4lX [0x%4lX]\n",
	...
	if(board_info->me4000_regbase == 0){
		printk(KERN_ERR"ME4000:get_registers():PCI base address 2 is not available\n");
		return -ENODEV;
	}
	...
	if(board_info->timer_regbase == 0){
		printk(KERN_ERR"ME4000:get_registers():PCI base address 3 is not available\n");
		return -ENODEV;
	}
	board_info->timer_regbase_size = pci_resource_len(dev, 3);

	PDEBUG("get_registers():Timer registers at address 0x%4lX [0x%4lX]\n",
			board_info->timer_regbase,
			board_info->timer_regbase_size);
	...
	if(board_info->program_regbase == 0){
		printk(KERN_ERR"get_registers():ME4000:PCI base address 5 is not available\n");
		return -ENODEV;
	}
	board_info->program_regbase_size = pci_resource_len(dev, 5);

	PDEBUG("get_registers():Program registers at address 0x%4lX [0x%4lX]\n",
			board_info->program_regbase,
			board_info->program_regbase_size);
...
static int init_board_info(struct pci_dev *pci_dev_p, me4000_info_t *board_info){
	int i;
	int result;
	struct list_head *board_p;
	...
	if(i == ME4000_BOARD_VERSIONS){
		printk(KERN_ERR"ME4000:init_board_info():Device ID not valid\n");
		return -ENODEV;
	}

	/* Get the index of the board in the global list */
	for(board_p = me4000_board_info_list.next, i = 0; board_p != &me4000_board_info_list; board_p = board_p->next, i++){
	...
	if(board_p == &me4000_board_info_list){
		printk(KERN_ERR"ME4000:init_board_info():Cannot get index of baord\n");
		return -ENODEV;
	}
	...
	if(result != PCIBIOS_SUCCESSFUL){
		printk(KERN_WARNING"ME4000:init_board_info: Can't get serial_no\n");
		return result;
	}
	PDEBUG("init_board_info():serial_no = 0x%x\n", board_info->serial_no);

	/* Get the hardware revision */
	result = pci_read_config_byte(pci_dev_p, 0x08, &board_info->hw_revision);
	if(result != PCIBIOS_SUCCESSFUL){
		printk(KERN_WARNING"ME4000:init_board_info():Can't get hw_revision\n");
		return result;
	}
...
static int alloc_ao_contexts(me4000_info_t *info){
	...
	for(i = 0; i < info->board_p->ao.count; i++){
		...
		if(!ao_context){
			printk(KERN_ERR"alloc_ao_contexts():Can't get memory for ao context\n");
		...
		if(info->board_p->ao.fifo_count){
			...
			ao_context->circ_buf.buf = kmalloc(ME4000_AO_BUFFER_SIZE, GFP_KERNEL);
			if(!ao_context->circ_buf.buf){
				printk(KERN_ERR"alloc_ao_contexts():Can't get circular buffer\n");
				release_ao_contexts(info);
				return -ENOMEM;
			}
			memset(ao_context->circ_buf.buf, 0, ME4000_AO_BUFFER_SIZE);
		...
		switch(i){
			case 0:
				ao_context->ctrl_reg = info->me4000_regbase + ME4000_AO_00_CTRL_REG;
				ao_context->status_reg = info->me4000_regbase + ME4000_AO_00_STATUS_REG;
				ao_context->fifo_reg = info->me4000_regbase + ME4000_AO_00_FIFO_REG;
				ao_context->single_reg = info->me4000_regbase + ME4000_AO_00_SINGLE_REG;
				ao_context->timer_reg = info->me4000_regbase + ME4000_AO_00_TIMER_REG;
				ao_context->irq_status_reg = info->me4000_regbase + ME4000_IRQ_STATUS_REG;
				ao_context->preload_reg = info->me4000_regbase + ME4000_AO_LOADSETREG_XX;
				break;
			case 1:
				ao_context->ctrl_reg = info->me4000_regbase + ME4000_AO_01_CTRL_REG;
				ao_context->status_reg = info->me4000_regbase + ME4000_AO_01_STATUS_REG;
				ao_context->fifo_reg = info->me4000_regbase + ME4000_AO_01_FIFO_REG;
				ao_context->single_reg = info->me4000_regbase + ME4000_AO_01_SINGLE_REG;
				ao_context->timer_reg = info->me4000_regbase + ME4000_AO_01_TIMER_REG;
				ao_context->irq_status_reg = info->me4000_regbase + ME4000_IRQ_STATUS_REG;
				ao_context->preload_reg = info->me4000_regbase + ME4000_AO_LOADSETREG_XX;
				break;
			case 2:
				ao_context->ctrl_reg = info->me4000_regbase + ME4000_AO_02_CTRL_REG;
				ao_context->status_reg = info->me4000_regbase + ME4000_AO_02_STATUS_REG;
				ao_context->fifo_reg = info->me4000_regbase + ME4000_AO_02_FIFO_REG;
				ao_context->single_reg = info->me4000_regbase + ME4000_AO_02_SINGLE_REG;
				ao_context->timer_reg = info->me4000_regbase + ME4000_AO_02_TIMER_REG;
				ao_context->irq_status_reg = info->me4000_regbase + ME4000_IRQ_STATUS_REG;
				ao_context->preload_reg = info->me4000_regbase + ME4000_AO_LOADSETREG_XX;
				break;
			case 3:
				ao_context->ctrl_reg = info->me4000_regbase + ME4000_AO_03_CTRL_REG;
				ao_context->status_reg = info->me4000_regbase + ME4000_AO_03_STATUS_REG;
				ao_context->fifo_reg = info->me4000_regbase + ME4000_AO_03_FIFO_REG;
				ao_context->single_reg = info->me4000_regbase + ME4000_AO_03_SINGLE_REG;
				ao_context->timer_reg = info->me4000_regbase + ME4000_AO_03_TIMER_REG;
				ao_context->irq_status_reg = info->me4000_regbase + ME4000_IRQ_STATUS_REG;
				ao_context->preload_reg = info->me4000_regbase + ME4000_AO_LOADSETREG_XX;
				break;
			default:
		...
		if(info->board_p->ao.fifo_count){
			...
			err = request_irq(ao_context->irq, me4000_ao_isr, IRQF_DISABLED | IRQF_SHARED, ME4000_NAME, ao_context);
			if(err){
				printk(KERN_ERR"alloc_ao_contexts():Can't get interrupt line");
				if(ao_context->circ_buf.buf) kfree(ao_context->circ_buf.buf);
				kfree(ao_context);
				release_ao_contexts(info);
				return -ENODEV;
...
static int alloc_ai_context(me4000_info_t *info){
	...
	if(info->board_p->ai.count){
		...
		if(!ai_context){
			printk(KERN_ERR"ME4000:alloc_ai_context():Can't get memory for ai context\n");
			return -ENOMEM;
		}
		...
		ai_context->ctrl_reg = info->me4000_regbase + ME4000_AI_CTRL_REG;
		ai_context->status_reg = info->me4000_regbase + ME4000_AI_STATUS_REG;
		ai_context->channel_list_reg = info->me4000_regbase + ME4000_AI_CHANNEL_LIST_REG;
		ai_context->data_reg = info->me4000_regbase + ME4000_AI_DATA_REG;
		ai_context->chan_timer_reg = info->me4000_regbase + ME4000_AI_CHAN_TIMER_REG;
		ai_context->chan_pre_timer_reg = info->me4000_regbase + ME4000_AI_CHAN_PRE_TIMER_REG;
		ai_context->scan_timer_low_reg = info->me4000_regbase + ME4000_AI_SCAN_TIMER_LOW_REG;
		ai_context->scan_timer_high_reg = info->me4000_regbase + ME4000_AI_SCAN_TIMER_HIGH_REG;
		ai_context->scan_pre_timer_low_reg = info->me4000_regbase + ME4000_AI_SCAN_PRE_TIMER_LOW_REG;
		ai_context->scan_pre_timer_high_reg = info->me4000_regbase + ME4000_AI_SCAN_PRE_TIMER_HIGH_REG;
		ai_context->start_reg = info->me4000_regbase + ME4000_AI_START_REG;
		ai_context->irq_status_reg = info->me4000_regbase + ME4000_IRQ_STATUS_REG;
		ai_context->sample_counter_reg = info->me4000_regbase + ME4000_AI_SAMPLE_COUNTER_REG;
	}
...
static int alloc_dio_context(me4000_info_t *info){
	...
	if(info->board_p->dio.count){
		...
		if(!dio_context){
			printk(KERN_ERR"ME4000:alloc_dio_context():Can't get memory for dio context\n");
			return -ENOMEM;
		}
		...
		dio_context->dir_reg = info->me4000_regbase + ME4000_DIO_DIR_REG;
		dio_context->ctrl_reg = info->me4000_regbase + ME4000_DIO_CTRL_REG;
		dio_context->port_0_reg = info->me4000_regbase + ME4000_DIO_PORT_0_REG;
		dio_context->port_1_reg = info->me4000_regbase + ME4000_DIO_PORT_1_REG;
		dio_context->port_2_reg = info->me4000_regbase + ME4000_DIO_PORT_2_REG;
		dio_context->port_3_reg = info->me4000_regbase + ME4000_DIO_PORT_3_REG;
	}
...
static int alloc_cnt_context(me4000_info_t *info){
	...
	if(info->board_p->cnt.count){
		...
		if(!cnt_context){
			printk(KERN_ERR"ME4000:alloc_cnt_context():Can't get memory for cnt context\n");
			return -ENOMEM;
		}
		...
		cnt_context->ctrl_reg = info->timer_regbase + ME4000_CNT_CTRL_REG;
		cnt_context->counter_0_reg = info->timer_regbase + ME4000_CNT_COUNTER_0_REG;
		cnt_context->counter_1_reg = info->timer_regbase + ME4000_CNT_COUNTER_1_REG;
		cnt_context->counter_2_reg = info->timer_regbase + ME4000_CNT_COUNTER_2_REG;
	}
...
static int alloc_ext_int_context(me4000_info_t *info){
	...
	if(info->board_p->cnt.count){
		ext_int_context = kmalloc(sizeof(me4000_ext_int_context_t), GFP_KERNEL);
		if(!ext_int_context){
			printk(KERN_ERR"ME4000:alloc_ext_int_context():Can't get memory for cnt context\n");
			return -ENOMEM;
		}
		...
		ext_int_context->ctrl_reg = info->me4000_regbase + ME4000_AI_CTRL_REG;
		ext_int_context->irq_status_reg = info->me4000_regbase + ME4000_IRQ_STATUS_REG;
	}
...
static int me4000_probe(struct pci_dev *dev, const struct pci_device_id *id){
	...
	if(!board_info){
		printk(KERN_ERR"ME4000:Can't get memory for board info structure\n");
		result = -ENOMEM;
		goto PROBE_ERROR_1;
	}
	...
	if (result){
		printk(KERN_ERR"me4000_probe():Cannot allocate ext_int context\n");
		goto PROBE_ERROR_7;
	}
...
static int me4000_xilinx_download(me4000_info_t *info){
	...
	for(idx = 0; idx < size; idx++){
		...
		if(inl(info->plx_regbase + PLX_ICR) & 0x20){
			printk(KERN_ERR"me4000_xilinx_download():Xilinx is still busy (idx = %d)\n", idx);
			return -EIO;
		}
	}
	...
	if (inl(info->plx_regbase + PLX_ICR) & 0x4){
		...
		printk(KERN_ERR"ME4000:me4000_xilinx_download():DONE flag is not set\n");
...
static int me4000_reset_board(me4000_info_t *info){
	...
	me4000_outl(ME4000_AI_CTRL_BIT_IMMEDIATE_STOP | ME4000_AI_CTRL_BIT_STOP, info->me4000_regbase + ME4000_AI_CTRL_REG);

	/* Set both stop bits in the analog output control register */
	me4000_outl(ME4000_AO_CTRL_BIT_IMMEDIATE_STOP | ME4000_AO_CTRL_BIT_STOP, info->me4000_regbase + ME4000_AO_00_CTRL_REG);
	me4000_outl(ME4000_AO_CTRL_BIT_IMMEDIATE_STOP | ME4000_AO_CTRL_BIT_STOP, info->me4000_regbase + ME4000_AO_01_CTRL_REG);
	me4000_outl(ME4000_AO_CTRL_BIT_IMMEDIATE_STOP | ME4000_AO_CTRL_BIT_STOP, info->me4000_regbase + ME4000_AO_02_CTRL_REG);
	me4000_outl(ME4000_AO_CTRL_BIT_IMMEDIATE_STOP | ME4000_AO_CTRL_BIT_STOP, info->me4000_regbase + ME4000_AO_03_CTRL_REG);
	...
	me4000_outl(ME4000_AO_DEMUX_ADJUST_VALUE, info->me4000_regbase + ME4000_AO_DEMUX_ADJUST_REG);
...
static int me4000_open(struct inode *inode_p, struct file *file_p){
	...
	if(MAJOR(inode_p->i_rdev) == me4000_ao_major_driver_no){
		...
		for(ptr = me4000_board_info_list.next, i = 0; ptr != &me4000_board_info_list; ptr = ptr->next, i++){
			board_info = list_entry(ptr, me4000_info_t, list);
			if(i == board) break;
		}

		if(ptr == &me4000_board_info_list){
			printk(KERN_ERR"ME4000:me4000_open():Board %d not in device list\n", board);
			return -ENODEV;
		}

		/* Search for the dac context */
		for(ptr = board_info->ao_context_list.next, i = 0; ptr != &board_info->ao_context_list; ptr = ptr->next, i++){
			ao_context = list_entry(ptr, me4000_ao_context_t, list);
			if(i == dev) break;
		}

		if(ptr == &board_info->ao_context_list){
			printk(KERN_ERR"ME4000:me4000_open():Device %d not in device list\n", dev);
			return -ENODEV;
		}

		/* Check if mode is valid */
		if(mode > 2){
			printk(KERN_ERR"ME4000:me4000_open():Mode is not valid\n");
			return -ENODEV;
		}

		/* Check if mode is valid for this AO */
		if((mode != ME4000_AO_CONV_MODE_SINGLE) && (dev >= board_info->board_p->ao.fifo_count)){
			printk(KERN_ERR"ME4000:me4000_open():AO %d only in single mode available\n", dev);
			return -ENODEV;
		}
		...
		if(ao_context->dac_in_use){
			printk(KERN_ERR"ME4000:me4000_open():AO %d already in use\n", dev);
	...
	/* Analog input */
		...
		if(ai_context->in_use){
			printk(KERN_ERR"ME4000:me4000_open():AI already in use\n");
	...
	/* Digital I/O */
		...
		/* Search for the board context */
		for(ptr = me4000_board_info_list.next; ptr != &me4000_board_info_list; ptr = ptr->next){
			board_info = list_entry(ptr, me4000_info_t, list);
			if(board_info->board_count == board) break;
		}
		...
		if(dio_context->in_use){
			printk(KERN_ERR"ME4000:me4000_open():DIO already in use\n");
	...
	/* Counters */
		...
		if(cnt_context->in_use){
			printk(KERN_ERR"ME4000:me4000_open():CNT already in use\n");
	...
	/* External Interrupt */
		...
		if(ext_int_context->in_use){
			printk(KERN_ERR"ME4000:me4000_open():External interrupt already in use\n");
		...
		/* Request the interrupt line */
		err = request_irq(ext_int_context->irq, me4000_ext_int_isr, IRQF_DISABLED | IRQF_SHARED, ME4000_NAME, ext_int_context);
		if(err){
			printk(KERN_ERR"ME4000:me4000_open():Can't get interrupt line");
...
static int me4000_release(struct inode *inode_p, struct file *file_p){
	...
	if(MAJOR(inode_p->i_rdev) == me4000_ao_major_driver_no){
		...
		printk(KERN_ERR"ME4000:me4000_release():Major number unknown\n");
		return -EINVAL;
	}
...
static int me4000_ao_prepare(me4000_ao_context_t *ao_context){
	...
	if(ao_context->mode == ME4000_AO_CONV_MODE_CONTINUOUS){
		...
		if((mode & ME4000_AO_CONV_MODE_CONTINUOUS) && (mode & ME4000_AO_CTRL_BIT_ENABLE_FIFO)){
			return 0;
		}
		...
		me4000_outl(ME4000_AO_CONV_MODE_CONTINUOUS | ME4000_AO_CTRL_BIT_ENABLE_FIFO |
				ME4000_AO_CTRL_BIT_STOP | ME4000_AO_CTRL_BIT_IMMEDIATE_STOP, ao_context->ctrl_reg);
		...
		/* Only do anything if not already in the correct mode */
		unsigned long mode = me4000_inl(ao_context->ctrl_reg);
		if((mode & ME4000_AO_CONV_MODE_WRAPAROUND) && (mode & ME4000_AO_CTRL_BIT_ENABLE_FIFO)){
			return 0;
		}
		...
		/* Set the control register to default state  */
		spin_lock_irqsave(&ao_context->int_lock, flags);
		me4000_outl(ME4000_AO_CONV_MODE_WRAPAROUND | ME4000_AO_CTRL_BIT_ENABLE_FIFO |
				ME4000_AO_CTRL_BIT_STOP | ME4000_AO_CTRL_BIT_IMMEDIATE_STOP, ao_context->ctrl_reg);
		spin_unlock_irqrestore(&ao_context->int_lock, flags);
		...
		/* Only do anything if not already in the correct mode */
		unsigned long mode = me4000_inl(ao_context->ctrl_reg);
		if(!(mode & (ME4000_AO_CONV_MODE_WRAPAROUND | ME4000_AO_CONV_MODE_CONTINUOUS))){
			return 0;
		}
		...
		printk(KERN_ERR"ME4000:me4000_ao_prepare():Invalid mode specified\n");
		return -EINVAL;
	}
...
static ssize_t me4000_ao_write_sing(struct file *filep, const char *buff, size_t cnt, loff_t *offp){
	...
	if(cnt != 2){
		printk(KERN_ERR"me4000_ao_write_sing():Write count is not 2\n");
		return -EINVAL;
	}

	if(get_user(value, buffer)){
		printk(KERN_ERR"me4000_ao_write_sing():Cannot copy data from user\n");
		return -EFAULT;
	}
...
static ssize_t me4000_ao_write_wrap(struct file *filep, const char *buff, size_t cnt, loff_t *offp){
	me4000_ao_context_t *ao_context = filep->private_data;
	size_t i;
	u32 value;
	u32 tmp;
	...
	if(inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"ME4000:me4000_ao_write_wrap():There is already a conversion running\n");
		return -EBUSY;
	}

	if(count > ME4000_AO_FIFO_COUNT){
		printk(KERN_ERR"me4000_ao_write_wrap():Can't load more than %d values\n", ME4000_AO_FIFO_COUNT);
		return -ENOSPC;
	}
	...
	for(i = 0; i < count; i++){
		if(get_user(value, buffer + i)){
			printk(KERN_ERR"me4000_ao_write_single():Cannot copy data from user\n");
			return -EFAULT;
		}
		if(((ao_context->fifo_reg & 0xFF) == ME4000_AO_01_FIFO_REG) || ((ao_context->fifo_reg & 0xFF) == ME4000_AO_03_FIFO_REG))
			value = value << 16;
...
static ssize_t me4000_ao_write_cont(struct file *filep, const char *buff, size_t cnt, loff_t *offp){
	me4000_ao_context_t *ao_context = filep->private_data;
	const u16 *buffer = (const u16 *) buff;
	size_t count = cnt / 2;
	unsigned long flags;
	...
	if(filep->f_flags & O_APPEND){
		...
		while (count > 0) {
			if(filep->f_flags & O_NONBLOCK){
				if(ao_context->pipe_flag){
					printk(KERN_ERR"ME4000:me4000_ao_write_cont():Broken pipe in nonblocking write\n");
					return -EPIPE;
				}
				c = me4000_space_to_end(ao_context->circ_buf, ME4000_AO_BUFFER_COUNT);
				if(!c){
					PDEBUG("me4000_ao_write_cont():Returning from nonblocking write\n");
					break;
				}
			}
			else {
				wait_event_interruptible(ao_context->wait_queue,
						(c = me4000_space_to_end(ao_context->circ_buf, ME4000_AO_BUFFER_COUNT)));
				if(ao_context->pipe_flag){
					printk(KERN_ERR"me4000_ao_write_cont():Broken pipe in blocking write\n");
					return -EPIPE;
				}
				if(signal_pending(current)){
					printk(KERN_ERR"me4000_ao_write_cont():Wait for free buffer interrupted from signal\n");
					return -EINTR;
				}
			}
			...
			k = 2*c;
			k -= copy_from_user(ao_context->circ_buf.buf + ao_context->circ_buf.head, buffer, k);
			c = k/2;
			PDEBUG("me4000_ao_write_cont():Copy %d values from user space\n", c);

			if (!c) return -EFAULT;

			ao_context->circ_buf.head = (ao_context->circ_buf.head + c) & (ME4000_AO_BUFFER_COUNT - 1);
			buffer += c;
			count -= c;
			ret += c;
		...
		if(filep->f_flags & O_SYNC){
			while(inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM){
				...
				if(ao_context->pipe_flag){
					PDEBUG("me4000_ao_write_cont():Broken pipe detected after sync\n");
					return -EPIPE;
				}
				if(signal_pending(current)){
					printk(KERN_ERR"me4000_ao_write_cont():Wait on state machine after sync interrupted\n");
					return -EINTR;
				}
			}
		}
		...
		if((me4000_inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM)){
			printk(KERN_ERR"me4000_ao_write_cont():Can't Preload DAC FIFO while conversion is running\n");
			return -EBUSY;
		}
		...
		spin_lock_irqsave(&ao_context->int_lock, flags);
		tmp = me4000_inl(ao_context->ctrl_reg);
		tmp &= ~(ME4000_AO_CTRL_BIT_ENABLE_FIFO | ME4000_AO_CTRL_BIT_ENABLE_IRQ);
		me4000_outl(tmp, ao_context->ctrl_reg);
		tmp |= ME4000_AO_CTRL_BIT_ENABLE_FIFO;
		me4000_outl(tmp, ao_context->ctrl_reg);
		spin_unlock_irqrestore(&ao_context->int_lock, flags);
		...
		PDEBUG("me4000_ao_write_cont():Write %d values to DAC on 0x%lX\n", c, ao_context->fifo_reg);

		/* Write values to the fifo */
		for(i = 0; i < c; i++){
			if(get_user(svalue, buffer)) return -EFAULT;

			if(((ao_context->fifo_reg & 0xFF) == ME4000_AO_01_FIFO_REG) || ((ao_context->fifo_reg & 0xFF) == ME4000_AO_03_FIFO_REG)){
				lvalue = ((u32) svalue) << 16;
			}
			else lvalue = (u32) svalue;
		...
		while (1) {
			...
			if (c <= 0){
				PDEBUG("me4000_ao_write_cont():Count reached 0\n");
				break;
			}
			...
			k -= copy_from_user(ao_context->circ_buf.buf + ao_context->circ_buf.head, buffer, k);
			c = k/2;
			PDEBUG("me4000_ao_write_cont():Wrote %d values to buffer\n", c);
			...
			spin_lock_irqsave(&ao_context->int_lock, flags);
			if(me4000_buf_count(ao_context->circ_buf, ME4000_AO_BUFFER_COUNT)){
				PDEBUG("me4000_ao_write_cont():Enable Interrupts\n");
				tmp = me4000_inl(ao_context->ctrl_reg);
				tmp |= ME4000_AO_CTRL_BIT_ENABLE_IRQ;
				me4000_outl(tmp, ao_context->ctrl_reg);
			}
...
static int me4000_ao_fsync_cont(struct file *file_p, struct dentry *dentry_p, int datasync){
	me4000_ao_context_t *ao_context;
	wait_queue_head_t queue;
	...
	while(inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM){
		interruptible_sleep_on_timeout(&queue, 1);
		if(ao_context->pipe_flag){
			printk(KERN_ERR"me4000_ao_fsync_cont():Broken pipe detected\n");
			return -EPIPE;
		}

		if(signal_pending(current)){
			printk(KERN_ERR"me4000_ao_fsync_cont():Wait on state machine interrupted\n");
			return -EINTR;
		}
	}
...
static int me4000_ao_ioctl_sing(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_ao_context_t *ao_context;
	...
	switch(service){
		...
		case ME4000_GET_USER_INFO:
			return me4000_get_user_info((me4000_user_info_t *) arg, ao_context->board_info);
		...
		case ME4000_AO_SIMULTANEOUS_UPDATE:
			return me4000_ao_simultaneous_update((me4000_ao_channel_list_t *) arg, ao_context);
		case ME4000_AO_EX_TRIG_TIMEOUT:
			return me4000_ao_ex_trig_timeout((unsigned long *) arg, ao_context);
...
static int me4000_ao_ioctl_wrap(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_ao_context_t *ao_context;
...
static int me4000_ao_ioctl_cont(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_ao_context_t *ao_context;
	...
	switch(service){
		...
		case ME4000_AO_GET_FREE_BUFFER:
			return me4000_ao_get_free_buffer((unsigned long *) arg, ao_context);
		default:
			return -ENOTTY;
	}
	return 0;
...
static int me4000_ao_start(unsigned long *arg, me4000_ao_context_t *ao_context){
	...
	if(get_user(timeout, arg)){
		printk(KERN_ERR"me4000_ao_start():Cannot copy data from user\n");
		return -EFAULT;
	}
	...
	if((tmp & ME4000_AO_CTRL_BIT_ENABLE_EX_TRIG)){
		if(timeout){
			...
			while(!(inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM)){
				interruptible_sleep_on_timeout(&queue, 1);
				if(signal_pending(current)){
					printk(KERN_ERR"ME4000:me4000_ao_start():Wait on start of state machine interrupted\n");
					return -EINTR;
				}
				if(((jiffies - ref) > (timeout * HZ / USER_HZ))){ // 2.6 has diffrent definitions for HZ in user and kernel space
					printk(KERN_ERR"ME4000:me4000_ao_start():Timeout reached\n");
					return -EIO;
				}
			}
		}
...
static int me4000_ao_stop(me4000_ao_context_t *ao_context){
	...
	while(inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM){
		interruptible_sleep_on_timeout(&queue, 1);
		if(signal_pending(current)){
			printk(KERN_ERR"me4000_ao_stop():Wait on state machine after stop interrupted\n");
			return -EINTR;
		}
	}
...
static int me4000_ao_immediate_stop(me4000_ao_context_t *ao_context){
	...
	while(inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM){
		interruptible_sleep_on_timeout(&queue, 1);
		if(signal_pending(current)){
			printk(KERN_ERR"me4000_ao_immediate_stop():Wait on state machine after stop interrupted\n");
			return -EINTR;
		}
	}
...
static int me4000_ao_timer_set_divisor(u32 *arg, me4000_ao_context_t *ao_context){
	u32 divisor;
	u32 tmp;

	CALL_PDEBUG("me4000_ao_timer set_divisor() is executed\n");

	if(get_user(divisor, arg)) return -EFAULT;

	/* Check if the state machine is stopped */
	tmp = me4000_inl(ao_context->status_reg);
	if(tmp & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"me4000_ao_timer_set_divisor():Can't set timer while DAC is running\n");
		return -EBUSY;
	}
	...
	if(divisor < ME4000_AO_MIN_TICKS){
		printk(KERN_ERR"ME4000:me4000_ao_timer set_divisor():Divisor to low\n");
		return -EINVAL;
	}
...
static int me4000_ao_ex_trig_set_edge(int *arg, me4000_ao_context_t *ao_context){
	int mode;
	u32 tmp;
	unsigned long flags;
	...
	if(get_user(mode, arg)) return -EFAULT;

	/* Check if the state machine is stopped */
	tmp = me4000_inl(ao_context->status_reg);
	if(tmp & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"me4000_ao_ex_trig_set_edge():Can't set trigger while DAC is running\n");
		return -EBUSY;
	}
	...
	if(mode == ME4000_AO_TRIGGER_EXT_EDGE_RISING){
		spin_lock_irqsave(&ao_context->int_lock, flags);
		tmp = me4000_inl(ao_context->ctrl_reg);
		tmp &= ~(ME4000_AO_CTRL_BIT_EX_TRIG_EDGE | ME4000_AO_CTRL_BIT_EX_TRIG_BOTH);
		me4000_outl(tmp, ao_context->ctrl_reg);
		spin_unlock_irqrestore(&ao_context->int_lock, flags);
	}
...
static int me4000_ao_ex_trig_enable(me4000_ao_context_t *ao_context){
	...
	tmp = me4000_inl(ao_context->status_reg);
	if(tmp & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"me4000_ao_ex_trig_enable():Can't enable trigger while DAC is running\n");
		return -EBUSY;
	}
...
static int me4000_ao_ex_trig_disable(me4000_ao_context_t *ao_context){
	...
	tmp = me4000_inl(ao_context->status_reg);
	if(tmp & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"me4000_ao_ex_trig_disable():Can't disable trigger while DAC is running\n");
		return -EBUSY;
	}
...
static int me4000_ao_simultaneous_disable(me4000_ao_context_t *ao_context){
	...
	tmp = me4000_inl(ao_context->status_reg);
	if(tmp & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"me4000_ao_simultaneous_disable():Can't disable while DAC is running\n");
		return -EBUSY;
	}
...
static int me4000_ao_preload_update(me4000_ao_context_t *ao_context){
	...
	list_for_each(entry, &ao_context->board_info->ao_context_list){
		...
		if(!(ctrl & (ME4000_AO_CTRL_BIT_MODE_0 | ME4000_AO_CTRL_BIT_MODE_1 | ME4000_AO_CTRL_BIT_ENABLE_EX_TRIG))){
			if(!(tmp & (0x1 << (((me4000_ao_context_t *) entry)->index + 16)))){
				tmp &= ~(0x1 << (((me4000_ao_context_t *) entry)->index));
			}
		}
	}
	me4000_outl(tmp, ao_context->preload_reg);
...
static int me4000_ao_simultaneous_update(me4000_ao_channel_list_t *arg, me4000_ao_context_t *ao_context){
	int err;
	int i;
	u32 tmp;
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ao_simultaneous_update():Can't copy command\n");
		return -EFAULT;
	}

	channels.list = kmalloc(sizeof(unsigned long) * channels.count, GFP_KERNEL);
	if(!channels.list){
		printk(KERN_ERR"ME4000:me4000_ao_simultaneous_update():Can't get buffer\n");
		return -ENOMEM;
	}
	memset(channels.list, 0, sizeof(unsigned long) * channels.count);

	/* Copy channel list from user */
	err = copy_from_user(channels.list, arg->list, sizeof(unsigned long) * channels.count);
	if(err){
		printk(KERN_ERR"ME4000:me4000_ao_simultaneous_update():Can't copy list\n");
		kfree(channels.list);
		return -EFAULT;
	}
	...
	for(i = 0; i < channels.count; i++){
		if(channels.list[i] > ao_context->board_info->board_p->ao.count){
			spin_unlock(&ao_context->board_info->preload_lock);
			kfree(channels.list);
			printk(KERN_ERR"ME4000:me4000_ao_simultaneous_update():Invalid board number specified\n");
			return -EFAULT;
		}
...
static int me4000_ao_synchronous_ex_trig(me4000_ao_context_t *ao_context){
	...
	tmp = me4000_inl(ao_context->status_reg);
	if(tmp & ME4000_AO_STATUS_BIT_FSM){
		printk(KERN_ERR"me4000_ao_synchronous_ex_trig(): DAC is running\n");
		return -EBUSY;
	}
...
static int me4000_ao_get_free_buffer(unsigned long *arg, me4000_ao_context_t *ao_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ao_get_free_buffer():Can't copy to user space\n");
		return -EFAULT;
	}
...
static int me4000_ao_ex_trig_timeout(unsigned long *arg, me4000_ao_context_t *ao_context){
	...
	if(get_user(timeout, arg)){
		printk(KERN_ERR"me4000_ao_ex_trig_timeout():Cannot copy data from user\n");
		return -EFAULT;
	}
	...
	if((tmp & ME4000_AO_CTRL_BIT_ENABLE_EX_TRIG)){
		if(timeout){
			...
			while((inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM)){
				interruptible_sleep_on_timeout(&queue, 1);
				if(signal_pending(current)){
					printk(KERN_ERR"ME4000:me4000_ao_ex_trig_timeout():Wait on start of state machine interrupted\n");
					return -EINTR;
				}
				if(((jiffies - ref) > (timeout * HZ / USER_HZ))){ // 2.6 has diffrent definitions for HZ in user and kernel space
					printk(KERN_ERR"ME4000:me4000_ao_ex_trig_timeout():Timeout reached\n");
					return -EIO;
				}
			}
		}
		...
		printk(KERN_ERR"ME4000:me4000_ao_ex_trig_timeout():External Trigger is not enabled\n");
		return -EINVAL;
	}
...
static int me4000_ao_enable_do(me4000_ao_context_t *ao_context){
	...
	if(ao_context->index != 3){
		printk(KERN_ERR"me4000_ao_enable_do():Only available for analog output 3\n");
		return -ENOTTY;
	}
...
static int me4000_ao_disable_do(me4000_ao_context_t *ao_context){
	...
	/* Only available for analog output 3 */
	if(ao_context->index != 3){
		printk(KERN_ERR"me4000_ao_disable():Only available for analog output 3\n");
		return -ENOTTY;
	}
...
static int me4000_ao_fsm_state(int *arg, me4000_ao_context_t *ao_context){
	...
	tmp = (me4000_inl(ao_context->status_reg) & ME4000_AO_STATUS_BIT_FSM) ? 1 : 0;
...
static int me4000_ai_prepare(me4000_ai_context_t *ai_context){
	...
	me4000_outl(ai_context->mode | ME4000_AI_CTRL_BIT_STOP | ME4000_AI_CTRL_BIT_IMMEDIATE_STOP, ai_context->ctrl_reg);
	...
	if(ai_context->mode){
		/* Request the interrupt line */
		err = request_irq(ai_context->irq, me4000_ai_isr, IRQF_DISABLED | IRQF_SHARED, ME4000_NAME, ai_context);
		if(err){
			printk(KERN_ERR"ME4000:me4000_ai_prepare():Can't get interrupt line");
			return -ENODEV;
		}

		/* Allocate circular buffer */
		ai_context->circ_buf.buf = kmalloc(ME4000_AI_BUFFER_SIZE, GFP_KERNEL);
		if(!ai_context->circ_buf.buf){
			printk(KERN_ERR"ME4000:me4000_ai_prepare():Can't get circular buffer\n");
			free_irq(ai_context->irq, ai_context);
			return -ENOMEM;
		}
...
static int me4000_ai_reset(me4000_ai_context_t *ai_context){
	...
	while(inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_FSM){
		interruptible_sleep_on_timeout(&queue, 1);
		if(signal_pending(current)){
			printk(KERN_ERR"me4000_ai_reset():Wait on state machine after stop interrupted\n");
			return -EINTR;
		}
	}
	...
	spin_lock_irqsave(&ai_context->int_lock, flags);
	tmp = me4000_inl(ai_context->ctrl_reg);
	me4000_outl(ME4000_AI_CTRL_BIT_IMMEDIATE_STOP | ME4000_AI_CTRL_BIT_STOP, ai_context->ctrl_reg);
	spin_unlock_irqrestore(&ai_context->int_lock, flags);
...
static int me4000_ai_ioctl_sing(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_ai_context_t *ai_context;
	...
	if(_IOC_NR(service) > ME4000_IOCTL_MAXNR){
		printk(KERN_ERR"me4000_ai_ioctl_sing():Service number to high\n");
		return -ENOTTY;
	}
	...
	switch(service){
		...
		case ME4000_AI_EX_TRIG_SETUP:
			return me4000_ai_ex_trig_setup((me4000_ai_trigger_t *) arg, ai_context);
		case ME4000_GET_USER_INFO:
			return me4000_get_user_info((me4000_user_info_t *) arg, ai_context->board_info);
		...
		default:
			printk(KERN_ERR"me4000_ai_ioctl_sing():Invalid service number\n");
			return -ENOTTY;
	}
	return 0;
}
...
static int me4000_ai_single(me4000_ai_single_t *arg, me4000_ai_context_t *ai_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_single():Can't copy from user space\n");
		return -EFAULT;
	}
	...
	switch(cmd.range){
		...
		case ME4000_AI_LIST_RANGE_UNIPOLAR_2_5:
			break;
		default:
			printk(KERN_ERR"ME4000:me4000_ai_single():Invalid range specified\n");
			return -EINVAL;
	}
	...
	switch(cmd.mode){
		case ME4000_AI_LIST_INPUT_SINGLE_ENDED:
			if(cmd.channel >= ai_context->board_info->board_p->ai.count){
				printk(KERN_ERR"ME4000:me4000_ai_single():Analog input is not available\n");
				return -EINVAL;
			}
			break;
		case ME4000_AI_LIST_INPUT_DIFFERENTIAL:
			if(cmd.channel >= ai_context->board_info->board_p->ai.diff_count){
				printk(KERN_ERR"ME4000:me4000_ai_single():Analog input is not available in differential mode\n");
				return -EINVAL;
			}
			break;
		default:
			printk(KERN_ERR"ME4000:me4000_ai_single():Invalid mode specified\n");
			return -EINVAL;
	}
	...
	me4000_outl(cmd.channel | cmd.range | cmd.mode | ME4000_AI_LIST_LAST_ENTRY, ai_context->channel_list_reg);
	...
	if(tmp & ME4000_AI_CTRL_BIT_EX_TRIG){
		jiffy = jiffies;
		while(!(me4000_inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_EF_DATA)){
			interruptible_sleep_on_timeout(&queue, 1);
			if(signal_pending(current)){
				printk(KERN_ERR"ME4000:me4000_ai_single():Wait on start of state machine interrupted\n");
				return -EINTR;
			}
			if(((jiffies - jiffy) > (cmd.timeout * HZ / USER_HZ)) && cmd.timeout){ // 2.6 has diffrent definitions for HZ in user and kernel space
				printk(KERN_ERR"ME4000:me4000_ai_single():Timeout reached\n");
				return -EIO;
			}
		}
	}
		...
		/* Wait until ready */
		udelay(10);
		if(!(me4000_inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_EF_DATA)){
			printk(KERN_ERR"ME4000:me4000_ai_single():Value not available after wait\n");
			return -EIO;
		}
	}
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_single():Can't copy to user space\n");
		return -EFAULT;
	}
...
static int me4000_ai_ioctl_sw(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_ai_context_t *ai_context;
	...
	if(_IOC_NR(service) > ME4000_IOCTL_MAXNR){
		printk(KERN_ERR"me4000_ai_ioctl_sw():Service number to high\n");
		return -ENOTTY;
	}
	...
	switch(service){
		...
		case ME4000_AI_GET_COUNT_BUFFER:
			return me4000_ai_get_count_buffer((unsigned long *) arg, ai_context);
		default:
			printk(KERN_ERR"ME4000:me4000_ai_ioctl_sw():Invalid service number %d\n", service);
			return -ENOTTY;
	}
	return 0;
}
...
static int me4000_ai_ioctl_ext(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_ai_context_t *ai_context;
	...
	if(_IOC_NR(service) > ME4000_IOCTL_MAXNR){
		printk(KERN_ERR"me4000_ai_ioctl_ext():Service number to high\n");
		return -ENOTTY;
	}
	...
	switch(service){
		...
		case ME4000_AI_GET_COUNT_BUFFER:
			return me4000_ai_get_count_buffer((unsigned long *) arg, ai_context);
		default:
			printk(KERN_ERR"ME4000:me4000_ai_ioctl_ext():Invalid service number %d\n", service);
			return -ENOTTY;
	}
	return 0;
}
...
static int me4000_ai_config(me4000_ai_config_t *arg, me4000_ai_context_t *ai_context){
	...
	if(inl(ai_context->ctrl_reg) & ME4000_AI_STATUS_BIT_FSM){
		printk(KERN_ERR"ME4000:me4000_ai_config():Conversion is not stopped\n");
		err = -EBUSY;
		goto AI_CONFIG_ERR;
	}
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_config():Can't copy from user space\n");
		err = -EFAULT;
		goto AI_CONFIG_ERR;
	}

	PDEBUG("me4000_ai_config():chan = %ld, pre_chan = %ld, scan_low = %ld, scan_high = %ld, count = %ld\n",
			cmd.timer.chan, cmd.timer.pre_chan, cmd.timer.scan_low, cmd.timer.scan_high, cmd.channel_list.count);

	/* Check whether sample and hold is available for this board */
	if(cmd.sh){
		if(!ai_context->board_info->board_p->ai.sh_count){
			printk(KERN_ERR"ME4000:me4000_ai_config():Sample and Hold is not available for this board\n");
			err = -ENODEV;
			goto AI_CONFIG_ERR;
		}
	}
	...
	if(cmd.channel_list.count > ME4000_AI_CHANNEL_LIST_COUNT){
		printk(KERN_ERR"me4000_ai_config():Channel list is to large\n");
		err = -EINVAL;
		goto AI_CONFIG_ERR;
	}
	...
	if(!list){
		printk(KERN_ERR"ME4000:me4000_ai_config():Can't get memory for channel list\n");
		err = -ENOMEM;
		goto AI_CONFIG_ERR;
	}
	err = copy_from_user(list, cmd.channel_list.list, sizeof(u32) * cmd.channel_list.count);
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_config():Can't copy from user space\n");
		err = -EFAULT;
		goto AI_CONFIG_ERR;
	...
	if(!(list[cmd.channel_list.count - 1] & ME4000_AI_LIST_LAST_ENTRY)){
		printk(KERN_WARNING"me4000_ai_config():Last entry bit is not set\n");
	...
	for(i = 0; i < cmd.channel_list.count; i++){
		if((list[i] & 0x20) != mode){
			printk(KERN_ERR"ME4000:me4000_ai_config():Mode is not equal for all entries\n");
			err = -EINVAL;
			goto AI_CONFIG_ERR;
		}
	}
	...
	if(mode == ME4000_AI_LIST_INPUT_SINGLE_ENDED){
		for(i = 0; i < cmd.channel_list.count; i++){
			if((list[i] & 0x1F) >= ai_context->board_info->board_p->ai.count){
				printk(KERN_ERR"ME4000:me4000_ai_config():Channel is not available for single ended\n");
				err = -EINVAL;
				goto AI_CONFIG_ERR;
			}
		}
		...
		for(i = 0; i < cmd.channel_list.count; i++){
			if((list[i] & 0x1F) >= ai_context->board_info->board_p->ai.diff_count){
				printk(KERN_ERR"ME4000:me4000_ai_config():Channel is not available for differential\n");
				err = -EINVAL;
				goto AI_CONFIG_ERR;
			}
		}
	...
	if(mode == ME4000_AI_LIST_INPUT_DIFFERENTIAL){
		for(i = 0; i < cmd.channel_list.count; i++){
			if((list[i] & 0xC0) != ME4000_AI_LIST_RANGE_BIPOLAR_10 && (list[i] & 0xC0) != ME4000_AI_LIST_RANGE_BIPOLAR_2_5){
				printk(KERN_ERR"ME4000:me4000_ai_config():Bipolar is not selected in differential mode\n");
				err = -EINVAL;
				goto AI_CONFIG_ERR;
			}
		}
	...
	if(ai_context->mode != ME4000_AI_ACQ_MODE_EXT_SINGLE_VALUE){
		...
		if(cmd.timer.chan < ME4000_AI_MIN_TICKS){
			printk(KERN_ERR"ME4000:me4000_ai_config():Channel timer divisor is to low\n");
			err = -EINVAL;
			goto AI_CONFIG_ERR;
		}
		...
		if((cmd.sh) && (cmd.timer.chan != ME4000_AI_MIN_TICKS)){
			printk(KERN_ERR"ME4000:me4000_ai_config():Channel timer divisor must be at minimum when sample and hold is activated\n");
			err = -EINVAL;
			goto AI_CONFIG_ERR;
		}
		...
		if(cmd.timer.pre_chan < ME4000_AI_MIN_TICKS){
			printk(KERN_ERR"ME4000:me4000_ai_config():Channel pre timer divisor is to low\n");
			err = -EINVAL;
			goto AI_CONFIG_ERR;
		}
		...
		me4000_outl(cmd.timer.pre_chan - 1, ai_context->chan_pre_timer_reg);
		...
		if(ai_context->mode != ME4000_AI_ACQ_MODE_EXT_SINGLE_CHANLIST){
			...
			scan = (u64) cmd.timer.scan_low | ((u64) cmd.timer.scan_high << 32);
			if(scan != 0){
				if(scan < cmd.channel_list.count * cmd.timer.chan + 1){
					printk(KERN_ERR"ME4000:me4000_ai_config():Scan timer divisor is to low\n");
					err = -EINVAL;
					goto AI_CONFIG_ERR;
				}
			}
			...
			if(scan != 0){
				...
				me4000_outl(tmp, ai_context->scan_timer_low_reg);
				tmp = (u32) ((scan >> 32) & 0xFFFFFFFF);
				me4000_outl(tmp, ai_context->scan_timer_high_reg);

				scan = scan - (cmd.timer.chan - 1) + (cmd.timer.pre_chan - 1);
				tmp = (u32) (scan & 0xFFFFFFFF);
				me4000_outl(tmp, ai_context->scan_pre_timer_low_reg);
				tmp = (u32) ((scan >> 32) & 0xFFFFFFFF);
				me4000_outl(tmp, ai_context->scan_pre_timer_high_reg);
			}
			else {
				me4000_outl(0x0, ai_context->scan_timer_low_reg);
				me4000_outl(0x0, ai_context->scan_timer_high_reg);

				me4000_outl(0x0, ai_context->scan_pre_timer_low_reg);
				me4000_outl(0x0, ai_context->scan_pre_timer_high_reg);
			}
...
AI_CONFIG_ERR:
	...
	tmp = me4000_inl(ai_context->ctrl_reg);
	tmp &= ~(ME4000_AI_CTRL_BIT_CHANNEL_FIFO | ME4000_AI_CTRL_BIT_SAMPLE_HOLD);
...
static int ai_common_start(me4000_ai_context_t *ai_context){
	...
	if(tmp & ME4000_AI_STATUS_BIT_FSM){
		printk(KERN_ERR"ME4000:ai_common_start():Conversion is not stopped\n");
		return -EBUSY;
	}
	...
	tmp &= ~(ME4000_AI_CTRL_BIT_DATA_FIFO | ME4000_AI_CTRL_BIT_LE_IRQ |
			ME4000_AI_CTRL_BIT_HF_IRQ | ME4000_AI_CTRL_BIT_SC_IRQ | ME4000_AI_CTRL_BIT_SC_RELOAD);
	...
	else if(ai_context->sample_counter && ai_context->sample_counter_reload){
		if(ai_context->sample_counter <= ME4000_AI_FIFO_COUNT / 2){
			/* Enable only Sample Counter Interrupt */
			tmp |= ME4000_AI_CTRL_BIT_SC_IRQ | ME4000_AI_CTRL_BIT_SC_RELOAD;
		}
		else {
			/* Enable Half Full Interrupt and Sample Counter Interrupt */
			tmp |= ME4000_AI_CTRL_BIT_SC_IRQ | ME4000_AI_CTRL_BIT_HF_IRQ | ME4000_AI_CTRL_BIT_SC_RELOAD;
		}
	}
	else{
...
static int me4000_ai_start_ex(unsigned long *arg, me4000_ai_context_t *ai_context){
	int err;
	wait_queue_head_t queue;
	unsigned long ref;
	unsigned long timeout;
	...
	if(get_user(timeout, arg)){
		printk(KERN_ERR"me4000_ai_start_ex():Cannot copy data from user\n");
		return -EFAULT;
	}
	...
	if(timeout){
		...
		while(!(inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_FSM)){
			interruptible_sleep_on_timeout(&queue, 1);
			if(signal_pending(current)){
				printk(KERN_ERR"ME4000:me4000_ai_start_ex():Wait on start of state machine interrupted\n");
				return -EINTR;
			}
			if(((jiffies - ref) > (timeout * HZ / USER_HZ))){ // 2.6 has diffrent definitions for HZ in user and kernel space
				printk(KERN_ERR"ME4000:me4000_ai_start_ex():Timeout reached\n");
				return -EIO;
			}
		}
	}
...
static int me4000_ai_stop(me4000_ai_context_t *ai_context){
	...
	spin_lock_irqsave(&ai_context->int_lock, flags);
	tmp = me4000_inl(ai_context->ctrl_reg);
	tmp &= ~(ME4000_AI_CTRL_BIT_HF_IRQ | ME4000_AI_CTRL_BIT_SC_IRQ | ME4000_AI_CTRL_BIT_DATA_FIFO);
	...
	while(inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_FSM){
		interruptible_sleep_on_timeout(&queue, 1);
		if(signal_pending(current)){
			printk(KERN_ERR"ME4000:me4000_ai_stop():Wait on state machine after stop interrupted\n");
			return -EINTR;
		}
	}
...
static int me4000_ai_ex_trig_setup(me4000_ai_trigger_t *arg, me4000_ai_context_t *ai_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_ex_trig_setup():Can't copy from user space\n");
		return -EFAULT;
	}
	...
	if(cmd.mode == ME4000_AI_TRIGGER_EXT_DIGITAL){
		...
		if(!ai_context->board_info->board_p->ai.ex_trig_analog){
			printk(KERN_ERR"ME4000:me4000_ai_ex_trig_setup():No analog trigger available\n");
			return -EINVAL;
		}
		tmp |= ME4000_AI_CTRL_BIT_EX_TRIG_ANALOG;
	}
		...
		spin_unlock_irqrestore(&ai_context->int_lock, flags);
		printk(KERN_ERR"ME4000:me4000_ai_ex_trig_setup():Invalid trigger mode specified\n");
		return -EINVAL;
	}

	if(cmd.edge == ME4000_AI_TRIGGER_EXT_EDGE_RISING){
		tmp &= ~(ME4000_AI_CTRL_BIT_EX_TRIG_BOTH | ME4000_AI_CTRL_BIT_EX_TRIG_FALLING);
	}
		...
		spin_unlock_irqrestore(&ai_context->int_lock, flags);
		printk(KERN_ERR"ME4000:me4000_ai_ex_trig_setup():Invalid trigger edge specified\n");
		return -EINVAL;
	}
...
static int me4000_ai_sc_setup(me4000_ai_sc_t *arg, me4000_ai_context_t *ai_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_sc_setup():Can't copy from user space\n");
		return -EFAULT;
	}
...
static ssize_t me4000_ai_read(struct file *filep, char *buff, size_t cnt, loff_t *offp){
	...
	while(count > 0) {
		if(filep->f_flags & O_NONBLOCK){
			c = me4000_values_to_end(ai_context->circ_buf, ME4000_AI_BUFFER_COUNT);
			if(!c){
				PDEBUG("me4000_ai_read():Returning from nonblocking read\n");
				break;
			}
		}
		else {
			...
			if(!(me4000_inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_FSM)){
				printk(KERN_ERR"ME4000:me4000_ai_read():Conversion interrupted\n");
				return -EPIPE;
			}

			wait_event_interruptible(ai_context->wait_queue,
					(me4000_values_to_end(ai_context->circ_buf, ME4000_AI_BUFFER_COUNT)));
			if(signal_pending(current)){
				printk(KERN_ERR"ME4000:me4000_ai_read():Wait on values interrupted from signal\n");
				return -EINTR;
			}
		}
		...
		k = 2*c;
		k -= copy_to_user(buffer, ai_context->circ_buf.buf + ai_context->circ_buf.tail, k);
		c = k/2;
		if (!c){
			printk(KERN_ERR"ME4000:me4000_ai_read():Cannot copy new values to user\n");
			return -EFAULT;
		}

		ai_context->circ_buf.tail = (ai_context->circ_buf.tail + c) & (ME4000_AI_BUFFER_COUNT - 1);
		buffer += c;
		count -= c;
		ret += c;
	...
	/* Check if conversion is still running */
	if(!(me4000_inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_FSM)){
		printk(KERN_ERR"ME4000:me4000_ai_read():Conversion not running after complete read\n");
		return -EPIPE;
	}
...
static int me4000_ai_fsm_state(int *arg, me4000_ai_context_t *ai_context){
	...
	tmp = (me4000_inl(ai_context->status_reg) & ME4000_AI_STATUS_BIT_FSM) ? 1 : 0;
...
static int me4000_ai_get_count_buffer(unsigned long *arg, me4000_ai_context_t *ai_context){
	...
	err = copy_to_user(arg, &c, sizeof(unsigned long));
	if(err){
		printk(KERN_ERR"ME4000:me4000_ai_get_count_buffer():Can't copy to user space\n");
		return -EFAULT;
	}
...
static unsigned short eeprom_read_cmd(me4000_ai_context_t *ai_context, unsigned long cmd, int length){
	...
	for(i = 0; i < 16; i++){
		...
		if(me4000_inl(ai_context->board_info->plx_regbase + PLX_ICR) & PLX_ICR_BIT_EEPROM_READ){
			id |= (0x8000 >> i);
...
static int me4000_eeprom_write(me4000_eeprom_t *arg, me4000_ai_context_t *ai_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_eeprom_write():Cannot copy from user\n");
		return err;
	}

	/* Enable writing */
	eeprom_write_cmd(ai_context, ME4000_EEPROM_CMD_WRITE_ENABLE, ME4000_EEPROM_CMD_LENGTH_WRITE_ENABLE);

	/* Command for date */
	date_high = (setup.date & 0xFFFF0000) >> 16;
	date_low = (setup.date & 0x0000FFFF);

	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_DATE_HIGH << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) date_high);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_DATE_LOW << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) date_low);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for unipolar 10V offset */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_1_UNI_OFFSET << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.uni_10_offset);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for unipolar 10V fullscale */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_1_UNI_FULLSCALE << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.uni_10_fullscale);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for unipolar 2,5V offset */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_4_UNI_OFFSET << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.uni_2_5_offset);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for unipolar 2,5V fullscale */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_4_UNI_FULLSCALE << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.uni_2_5_fullscale);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for bipolar 10V offset */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_1_BI_OFFSET << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.bi_10_offset);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for bipolar 10V fullscale */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_1_BI_FULLSCALE << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.bi_10_fullscale);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for bipolar 2,5V offset */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_4_BI_OFFSET << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.bi_2_5_offset);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for bipolar 2,5V fullscale */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_4_BI_FULLSCALE << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.bi_2_5_fullscale);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for differential 10V offset */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_1_DIFF_OFFSET << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.diff_10_offset);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for differential 10V fullscale */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_1_DIFF_FULLSCALE << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.diff_10_fullscale);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for differential 2,5V offset */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_4_DIFF_OFFSET << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.diff_2_5_offset);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Command for differential 2,5V fullscale */
	cmd = ME4000_EEPROM_CMD_WRITE | (ME4000_EEPROM_ADR_GAIN_4_DIFF_FULLSCALE << ME4000_EEPROM_DATA_LENGTH) |
		(0xFFFF & (unsigned long) setup.diff_2_5_fullscale);
	err = eeprom_write_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_WRITE);
	if(err) return err;

	/* Disable writing */
	eeprom_write_cmd(ai_context, ME4000_EEPROM_CMD_WRITE_DISABLE, ME4000_EEPROM_CMD_LENGTH_WRITE_DISABLE);
...
static int me4000_eeprom_read(me4000_eeprom_t *arg, me4000_ai_context_t *ai_context){
	...
	setup.uni_10_fullscale = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	/* Command for unipolar 2,5V offset */
	cmd = ME4000_EEPROM_CMD_READ | ME4000_EEPROM_ADR_GAIN_4_UNI_OFFSET;
	setup.uni_2_5_offset = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	/* Command for unipolar 2,5V fullscale */
	cmd = ME4000_EEPROM_CMD_READ | ME4000_EEPROM_ADR_GAIN_4_UNI_FULLSCALE;
	setup.uni_2_5_fullscale = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	/* Command for bipolar 10V offset */
	cmd = ME4000_EEPROM_CMD_READ | ME4000_EEPROM_ADR_GAIN_1_BI_OFFSET;
	setup.bi_10_offset = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	/* Command for bipolar 10V fullscale */
	cmd = ME4000_EEPROM_CMD_READ | ME4000_EEPROM_ADR_GAIN_1_BI_FULLSCALE;
	setup.bi_10_fullscale = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	/* Command for bipolar 2,5V offset */
	cmd = ME4000_EEPROM_CMD_READ | ME4000_EEPROM_ADR_GAIN_4_BI_OFFSET;
	setup.bi_2_5_offset = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);
	...
	setup.diff_10_offset = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);
	...
	setup.diff_2_5_offset = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	/* Command for differential 2,5V fullscale */
	cmd = ME4000_EEPROM_CMD_READ | ME4000_EEPROM_ADR_GAIN_4_DIFF_FULLSCALE;
	setup.diff_2_5_fullscale = eeprom_read_cmd(ai_context, cmd, ME4000_EEPROM_CMD_LENGTH_READ);

	err = copy_to_user(arg, &setup, sizeof(setup));
	if(err){
		printk(KERN_ERR"ME4000:me4000_eeprom_read():Cannot copy to user\n");
		return err;
	}
...
static int me4000_dio_ioctl(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_dio_context_t *dio_context;
	...
	switch(service){
		case ME4000_DIO_CONFIG:
			return me4000_dio_config((me4000_dio_config_t *) arg, dio_context);
		case ME4000_DIO_SET_BYTE:
			return me4000_dio_set_byte((me4000_dio_byte_t *) arg, dio_context);
		case ME4000_DIO_GET_BYTE:
			return me4000_dio_get_byte((me4000_dio_byte_t *) arg, dio_context);
		case ME4000_DIO_RESET:
			return me4000_dio_reset(dio_context);
		default:
			printk(KERN_ERR"ME4000:me4000_dio_ioctl():Invalid service number %d\n", service);
			return -ENOTTY;
	}
	return 0;
}
...
static int me4000_dio_config(me4000_dio_config_t *arg, me4000_dio_context_t *dio_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_dio_config():Can't copy from user space\n");
		return -EFAULT;
	}

	/* Check port parameter */
	if(cmd.port >= dio_context->dio_count){
		printk(KERN_ERR"ME4000:me4000_dio_config():Port %d is not available\n", cmd.port);
		return -EINVAL;
	}

	PDEBUG("me4000_dio_config(): port %d, mode %d, function %d\n", cmd.port, cmd.mode, cmd.function);
	...
	if(cmd.port == ME4000_DIO_PORT_A){
		if(cmd.mode == ME4000_DIO_PORT_INPUT){
			...
			if(!(me4000_inl(dio_context->dir_reg) & 0x1)){
				printk(KERN_ERR"ME4000:me4000_dio_config():Cannot set to input on opto isolated versions\n");
				return -EIO;
			}
			...
			tmp = me4000_inl(dio_context->ctrl_reg);
			tmp &= ~(ME4000_DIO_CTRL_BIT_MODE_0 | ME4000_DIO_CTRL_BIT_MODE_1 | ME4000_DIO_CTRL_BIT_FIFO_HIGH_0);
			tmp |= ME4000_DIO_CTRL_BIT_MODE_0 | ME4000_DIO_CTRL_BIT_MODE_1;
			me4000_outl(tmp, dio_context->ctrl_reg);
		}
			...
			printk(KERN_ERR"ME4000:me4000_dio_config():Mode %d is not available\n", cmd.mode);
			return -EINVAL;
		}
	}
		...
		if(cmd.mode == ME4000_DIO_PORT_INPUT){
			...
			/* Check if opto isolated version */
			if(!(me4000_inl(dio_context->dir_reg) & 0x1)){
				printk(KERN_ERR"ME4000:me4000_dio_config():Cannot set to output on opto isolated versions\n");
				return -EIO;
			}
			...
			/* Check if opto isolated version */
			if(!(me4000_inl(dio_context->dir_reg) & 0x1)){
				printk(KERN_ERR"ME4000:me4000_dio_config():Cannot set to FIFO low output on opto isolated versions\n");
				return -EIO;
			}

			tmp = me4000_inl(dio_context->ctrl_reg);
			tmp &= ~(ME4000_DIO_CTRL_BIT_MODE_2 | ME4000_DIO_CTRL_BIT_MODE_3 | ME4000_DIO_CTRL_BIT_FIFO_HIGH_1);
			tmp |= ME4000_DIO_CTRL_BIT_MODE_2 | ME4000_DIO_CTRL_BIT_MODE_3;
			me4000_outl(tmp, dio_context->ctrl_reg);
		}
		else if(cmd.mode == ME4000_DIO_FIFO_HIGH){
			/* Check if opto isolated version */
			if(!(me4000_inl(dio_context->dir_reg) & 0x1)){
				printk(KERN_ERR"ME4000:me4000_dio_config():Cannot set to FIFO high output on opto isolated versions\n");
				return -EIO;
			}
		...
		if(cmd.mode == ME4000_DIO_PORT_INPUT){
			...
			tmp = me4000_inl(dio_context->ctrl_reg);
			tmp &= ~(ME4000_DIO_CTRL_BIT_MODE_4 | ME4000_DIO_CTRL_BIT_MODE_5 | ME4000_DIO_CTRL_BIT_FIFO_HIGH_2);
			tmp |= ME4000_DIO_CTRL_BIT_MODE_4 | ME4000_DIO_CTRL_BIT_MODE_5;
			me4000_outl(tmp, dio_context->ctrl_reg);
		}
		else if(cmd.mode == ME4000_DIO_FIFO_HIGH){
		...
		if(cmd.mode == ME4000_DIO_PORT_INPUT){
			...
			tmp = me4000_inl(dio_context->ctrl_reg);
			tmp &= ~(ME4000_DIO_CTRL_BIT_MODE_6 | ME4000_DIO_CTRL_BIT_MODE_7 | ME4000_DIO_CTRL_BIT_FIFO_HIGH_3);
			tmp |= ME4000_DIO_CTRL_BIT_MODE_6 | ME4000_DIO_CTRL_BIT_MODE_7;
			me4000_outl(tmp, dio_context->ctrl_reg);
		}
		else if(cmd.mode == ME4000_DIO_FIFO_HIGH){
	...
	if((cmd.mode == ME4000_DIO_FIFO_HIGH) || (cmd.mode == ME4000_DIO_FIFO_LOW)){
		tmp = me4000_inl(dio_context->ctrl_reg);
		tmp &= ~(ME4000_DIO_CTRL_BIT_FUNCTION_0 | ME4000_DIO_CTRL_BIT_FUNCTION_1);
		if(cmd.function == ME4000_DIO_FUNCTION_PATTERN){
			...
			printk(KERN_ERR"ME4000:me4000_dio_config():Invalid port function specified\n");
			return -EINVAL;
		}
	}
...
static int me4000_dio_set_byte(me4000_dio_byte_t *arg, me4000_dio_context_t *dio_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_dio_set_byte():Can't copy from user space\n");
		return -EFAULT;
	}

	/* Check port parameter */
	if(cmd.port >= dio_context->dio_count){
		printk(KERN_ERR"ME4000:me4000_dio_set_byte():Port %d is not available\n", cmd.port);
		return -EINVAL;
	}

	if(cmd.port == ME4000_DIO_PORT_A){
		if((me4000_inl(dio_context->ctrl_reg) & 0x3) != 0x1){
			printk(KERN_ERR"ME4000:me4000_dio_set_byte():Port %d is not in output mode\n", cmd.port);
			return -EIO;
		}
...
static int me4000_dio_get_byte(me4000_dio_byte_t *arg, me4000_dio_context_t *dio_context){
	me4000_dio_byte_t cmd;
	int err;
	...
	err = copy_from_user(&cmd, arg, sizeof(me4000_dio_byte_t));
	if(err){
		printk(KERN_ERR"ME4000:me4000_dio_get_byte():Can't copy from user space\n");
		return -EFAULT;
	}

	/* Check port parameter */
	if(cmd.port >= dio_context->dio_count){
		printk(KERN_ERR"ME4000:me4000_dio_get_byte():Port %d is not available\n", cmd.port);
		return -EINVAL;
	}
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_dio_get_byte():Can't copy to user space\n");
		return -EFAULT;
	}
...
static int me4000_cnt_ioctl(struct inode * inode_p, struct file *file_p, unsigned int service, unsigned long arg){
	me4000_cnt_context_t *cnt_context;
	...
	switch(service){
		...
		case ME4000_CNT_CONFIG:
			return me4000_cnt_config((me4000_cnt_config_t *) arg, cnt_context);
...
static int me4000_cnt_config(me4000_cnt_config_t *arg, me4000_cnt_context_t *cnt_context){
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_cnt_config():Can't copy from user space\n");
		return -EFAULT;
	}
	...
	switch(cmd.counter){
		...
		case ME4000_CNT_COUNTER_2:
			...
			break;
		default:
			printk(KERN_ERR"ME4000:me4000_cnt_config():Counter %d is not available\n", cmd.counter);
			return -EINVAL;
	}
	...
	switch(cmd.mode){
		...
		case ME4000_CNT_MODE_5:
			...
			break;
		default:
			printk(KERN_ERR"ME4000:me4000_cnt_config():Mode %d is not available\n", cmd.mode);
			return -EINVAL;
	}
...
static int me4000_cnt_read(me4000_cnt_t *arg, me4000_cnt_context_t *cnt_context){
	me4000_cnt_t cmd;
	u8 tmp;
	int err;
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_cnt_read():Can't copy from user space\n");
		return -EFAULT;
	}
	...
	switch(cmd.counter){
		...
		case ME4000_CNT_COUNTER_2:
			...
			break;
		default:
			printk(KERN_ERR"ME4000:me4000_cnt_read():Counter %d is not available\n", cmd.counter);
			return -EINVAL;
	}
	...
	if(err){
		printk(KERN_ERR"ME4000:me4000_cnt_read():Can't copy to user space\n");
		return -EFAULT;
	}
...
static int me4000_cnt_write(me4000_cnt_t *arg, me4000_cnt_context_t *cnt_context){
	me4000_cnt_t cmd;
	u8 tmp;
	int err;
	...
	err = copy_from_user(&cmd, arg, sizeof(me4000_cnt_t));
	if(err){
		printk(KERN_ERR"ME4000:me4000_cnt_write():Can't copy from user space\n");
		return -EFAULT;
	}
	...
	switch(cmd.counter){
		...
		case ME4000_CNT_COUNTER_2:
			...
			break;
		default:
			printk(KERN_ERR"ME4000:me4000_cnt_write():Counter %d is not available\n", cmd.counter);
			return -EINVAL;
	}
...
static int me4000_ext_int_ioctl(struct inode * inode_p, struct file *file_p,
	...
	if(_IOC_NR(service) > ME4000_IOCTL_MAXNR){
		printk(KERN_ERR"me4000_ext_int_ioctl():Service number to high\n");
		return -ENOTTY;
	}
	...
	switch(service){
		...
		case ME4000_EXT_INT_COUNT:
			return me4000_ext_int_count((unsigned long *) arg, ext_int_context);
		default:
			printk(KERN_ERR"ME4000:me4000_ext_int_ioctl():Invalid service number %d\n", service);
			return -ENOTTY;
	}
	return 0;
}
...
static int me4000_get_user_info(me4000_user_info_t *arg, me4000_info_t *board_info){
...
static int me4000_ext_int_fasync(int fd, struct file *file_ptr, int mode){
	...
	result = fasync_helper(fd, file_ptr, mode, &ext_int_context->fasync_ptr);
...
static irqreturn_t me4000_ao_isr(int irq, void *dev_id)
{
	...
	if( ! ((0x1 << (ao_context->index + 3))  & me4000_inl(ao_context->irq_status_reg))){
	...
	if(!(tmp & ME4000_AO_STATUS_BIT_EF)&&(tmp & ME4000_AO_STATUS_BIT_HF)&&(tmp & ME4000_AO_STATUS_BIT_HF)){
		c = ME4000_AO_FIFO_COUNT;
		ISR_PDEBUG("me4000_ao_isr():Fifo empty\n");
	}
	else if((tmp & ME4000_AO_STATUS_BIT_EF)&&(tmp & ME4000_AO_STATUS_BIT_HF)&&(tmp & ME4000_AO_STATUS_BIT_HF)){
	...
	while(1){
		c1 = me4000_values_to_end(ao_context->circ_buf, ME4000_AO_BUFFER_COUNT);
		ISR_PDEBUG("me4000_ao_isr():Values to end = %d\n", c1);
		if(c1 > c) c1 = c;

		if(c1 <= 0){
			ISR_PDEBUG("me4000_ao_isr():Work done or buffer empty\n");
			break;
		}
		...
		if(((ao_context->fifo_reg & 0xFF) == ME4000_AO_01_FIFO_REG) ||
			...
			for(i = 0; i < c1; i++){
				value = ((u32)(*(ao_context->circ_buf.buf + ao_context->circ_buf.tail + i))) << 16;
				outl(value, ao_context->fifo_reg);
			}
		}
		else
			outsw(ao_context->fifo_reg, ao_context->circ_buf.buf + ao_context->circ_buf.tail, c1);

		//rdtscl(after);
		//printk(KERN_ERR"ME4000:me4000_ao_isr():Time lapse = %lu\n", after - before);

		ao_context->circ_buf.tail = (ao_context->circ_buf.tail + c1) & (ME4000_AO_BUFFER_COUNT - 1);
	...
	if(!me4000_buf_count(ao_context->circ_buf, ME4000_AO_BUFFER_COUNT)){
		ISR_PDEBUG("me4000_ao_isr():Disable Interrupt because no values left in buffer\n");
		tmp = me4000_inl(ao_context->ctrl_reg);
		tmp &= ~ME4000_AO_CTRL_BIT_ENABLE_IRQ;
		me4000_outl(tmp, ao_context->ctrl_reg);
	}
...
static irqreturn_t me4000_ai_isr(int irq, void *dev_id)
{
	...
	if(me4000_inl(ai_context->irq_status_reg) & ME4000_IRQ_STATUS_BIT_AI_HF){
		...
		if(!(tmp & ME4000_AI_STATUS_BIT_FF_DATA) &&
				!(tmp & ME4000_AI_STATUS_BIT_HF_DATA) && (tmp & ME4000_AI_STATUS_BIT_EF_DATA)){
			...
			c = 0;
			ISR_PDEBUG("me4000_ai_isr():Can't determine state of fifo\n");
		}

		ISR_PDEBUG("me4000_ai_isr():Try to read %d values\n", c);

		while(1){
			c1 = me4000_space_to_end(ai_context->circ_buf, ME4000_AI_BUFFER_COUNT);
			ISR_PDEBUG("me4000_ai_isr():Space to end = %d\n", c1);
			if(c1 > c) c1 = c;

			if(c1 <= 0){
				ISR_PDEBUG("me4000_ai_isr():Work done or buffer full\n");
				break;
			}

			insw(ai_context->data_reg, ai_context->circ_buf.buf + ai_context->circ_buf.head, c1);
			ai_context->circ_buf.head = (ai_context->circ_buf.head + c1) & (ME4000_AI_BUFFER_COUNT - 1);
			c -= c1;
		}

		/* Work is done, so reset the interrupt */
		ISR_PDEBUG("me4000_ai_isr():reset interrupt fifo half full interrupt\n");
		spin_lock(&ai_context->int_lock);
		tmp = me4000_inl(ai_context->ctrl_reg);
	...
	if(me4000_inl(ai_context->irq_status_reg) & ME4000_IRQ_STATUS_BIT_SC){
		...
		if(!ai_context->sample_counter_reload){
			ISR_PDEBUG("me4000_ai_isr():Single data block available\n");

			/* Poll data until fifo empty */
			for(i = 0; (i < ME4000_AI_FIFO_COUNT / 2) && (inl(ai_context->ctrl_reg) & ME4000_AI_STATUS_BIT_EF_DATA); i++){
				if(me4000_space_to_end(ai_context->circ_buf, ME4000_AI_BUFFER_COUNT)){
					* (ai_context->circ_buf.buf + ai_context->circ_buf.head) = inw(ai_context->data_reg);
					ai_context->circ_buf.head = (ai_context->circ_buf.head + 1) & (ME4000_AI_BUFFER_COUNT - 1);
				}
				else break;
			}
			...
			if(ai_context->sample_counter <= ME4000_AI_FIFO_COUNT / 2){
				ISR_PDEBUG("me4000_ai_isr():Interrupt from adjustable half full threshold\n");
				...
				if(!(tmp & ME4000_AI_STATUS_BIT_FF_DATA) &&
					...
					ISR_PDEBUG("me4000_ai_isr():Sample count values\n");
				}
				...
				ISR_PDEBUG("me4000_ai_isr():Multiple data block available\n");
				...
				if(!(tmp & ME4000_AI_STATUS_BIT_FF_DATA) &&
					...
					/* Poll data until fifo empty */
					for(i = 0; (i < ME4000_AI_FIFO_COUNT / 2) && (inl(ai_context->ctrl_reg) &
	...
	spin_lock(&ai_context->int_lock);
	if(!me4000_buf_space(ai_context->circ_buf, ME4000_AI_BUFFER_COUNT)){
		ISR_PDEBUG("me4000_ai_isr():Disable Interrupt because no space left in buffer\n");
		tmp = me4000_inl(ai_context->ctrl_reg);
		tmp &= ~(ME4000_AI_CTRL_BIT_SC_IRQ | ME4000_AI_CTRL_BIT_HF_IRQ | ME4000_AI_CTRL_BIT_LE_IRQ);
		me4000_outl(tmp, ai_context->ctrl_reg);
	}
	spin_unlock(&ai_context->int_lock);
...
static irqreturn_t me4000_ext_int_isr(int irq, void *dev_id)
{
	...
	if(me4000_inl(ext_int_context->irq_status_reg) & ME4000_IRQ_STATUS_BIT_EX){
		...
		if(ext_int_context->fasync_ptr){
			ISR_PDEBUG("me2600_ext_int_isr():Send signal to process\n");
			kill_fasync(&ext_int_context->fasync_ptr, SIGIO, POLL_IN);
		}
	}
...
static int me4000_read_procmem(char *buf, char **start, off_t offset, int count, int *eof, void *data){
	int len = 0;
	...
	/* Search for the board context */
	for(ptr = me4000_board_info_list.next; (ptr != &me4000_board_info_list) && (len < limit); ptr = ptr->next){
		board_info = list_entry(ptr, me4000_info_t, list);

		len += sprintf(buf + len, "Board number %d:\n", board_info->board_count);
		len += sprintf(buf + len, "---------------\n");
		len += sprintf(buf + len, "PLX base register = 0x%lX\n", board_info->plx_regbase);
		len += sprintf(buf + len, "PLX base register size = 0x%lX\n", board_info->plx_regbase_size);
		len += sprintf(buf + len, "ME4000 base register = 0x%lX\n", board_info->me4000_regbase);
		len += sprintf(buf + len, "ME4000 base register size = 0x%lX\n", board_info->me4000_regbase_size);
		len += sprintf(buf + len, "Serial number = 0x%X\n", board_info->serial_no);
		len += sprintf(buf + len, "Hardware revision = 0x%X\n", board_info->hw_revision);
		len += sprintf(buf + len, "Vendor id = 0x%X\n", board_info->vendor_id);
		len += sprintf(buf + len, "Device id = 0x%X\n", board_info->device_id);
		len += sprintf(buf + len, "PCI bus number = %d\n", board_info->pci_bus_no);
		len += sprintf(buf + len, "PCI device number = %d\n", board_info->pci_dev_no);
		len += sprintf(buf + len, "PCI function number = %d\n", board_info->pci_func_no);
		len += sprintf(buf + len, "IRQ = %u\n", board_info->irq);
		len += sprintf(buf + len, "Count of interrupts since module was loaded = %d\n", board_info->irq_count);

		len += sprintf(buf + len, "Count of analog outputs = %d\n", board_info->board_p->ao.count);
		len += sprintf(buf + len, "Count of analog output fifos = %d\n", board_info->board_p->ao.fifo_count);

		len += sprintf(buf + len, "Count of analog inputs = %d\n", board_info->board_p->ai.count);
		len += sprintf(buf + len, "Count of sample and hold devices for analog input = %d\n",
				board_info->board_p->ai.sh_count);
		len += sprintf(buf + len, "Analog external trigger available for analog input = %d\n",
				board_info->board_p->ai.ex_trig_analog);

		len += sprintf(buf + len, "Count of digital ports = %d\n", board_info->board_p->dio.count);

		len += sprintf(buf + len, "Count of counter devices = %d\n", board_info->board_p->cnt.count);
		len += sprintf(buf + len, "AI control register = 0x%08X\n", inl(board_info->me4000_regbase + ME4000_AI_CTRL_REG));

		len += sprintf(buf + len, "AO 0 control register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_00_CTRL_REG));
		len += sprintf(buf + len, "AO 0 status register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_00_STATUS_REG));
		len += sprintf(buf + len, "AO 1 control register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_01_CTRL_REG));
		len += sprintf(buf + len, "AO 1 status register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_01_STATUS_REG));
		len += sprintf(buf + len, "AO 2 control register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_02_CTRL_REG));
		len += sprintf(buf + len, "AO 2 status register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_02_STATUS_REG));
		len += sprintf(buf + len, "AO 3 control register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_03_CTRL_REG));
		len += sprintf(buf + len, "AO 3 status register = 0x%08X\n",
				inl(board_info->me4000_regbase + ME4000_AO_03_STATUS_REG));
	}
