	...
	if (pc->c[0] == REQUEST_SENSE) {
		...
		if (readpos[0] & 0x4) {
			printk(KERN_INFO "ide-tape: Block location is unknown"
					 "to the tape\n");
			clear_bit(IDE_DFLAG_ADDRESS_VALID, &drive->dev_flags);
			uptodate = 0;
		} else {
...
static void idetape_retry_pc(ide_drive_t *drive)
{
	...
	set_bit(IDE_DFLAG_IGNORE_DSC, &drive->dev_flags);
					  ...
					  struct request *rq, sector_t block)
{
	...
	if (!test_and_clear_bit(IDE_DFLAG_IGNORE_DSC, &drive->dev_flags) &&
	    (stat & SEEK_STAT) == 0) {
		if (postponed_rq == NULL) {
			tape->dsc_polling_start = jiffies;
...
static int idetape_wait_ready(ide_drive_t *drive, unsigned long timeout)
{
	...
	set_bit(IDE_DFLAG_MEDIUM_PRESENT, &drive->dev_flags);
	timeout += jiffies;
...
static void __ide_tape_discard_merge_buffer(ide_drive_t *drive)
{
	...
	clear_bit(IDE_DFLAG_FILEMARK, &drive->dev_flags);
...
static int idetape_add_chrdev_read_request(ide_drive_t *drive, int blocks)
{
	...
	if (test_bit(IDE_DFLAG_FILEMARK, &drive->dev_flags))
		return 0;
					...
					int mt_count)
{
	...
	if (tape->chrdev_dir == IDETAPE_DIR_READ) {
		tape->merge_bh_size = 0;
		if (test_and_clear_bit(IDE_DFLAG_FILEMARK, &drive->dev_flags))
			++count;
				   ...
				   size_t count, loff_t *ppos)
{
	...
	if (tape->chrdev_dir != IDETAPE_DIR_READ) {
		if (test_bit(IDE_DFLAG_DETECT_BS, &drive->dev_flags))
...
finish:
	if (!actually_read && test_bit(IDE_DFLAG_FILEMARK, &drive->dev_flags)) {
...
static int idetape_mtioctop(ide_drive_t *drive, short mt_op, int mt_count)
{
	...
	case MTOFFL:
		...
		retval = idetape_queue_pc_tail(drive, &pc);
		if (!retval)
			clear_bit(IDE_DFLAG_MEDIUM_PRESENT, &drive->dev_flags);
		return retval;
	case MTNOP:
		ide_tape_discard_merge_buffer(drive, 0);
	...
	case MTSETBLK:
		if (mt_count) {
			...
			clear_bit(IDE_DFLAG_DETECT_BS, &drive->dev_flags);
		} else
			set_bit(IDE_DFLAG_DETECT_BS, &drive->dev_flags);
		return 0;
	case MTSEEK:
		ide_tape_discard_merge_buffer(drive, 0);
...
static int idetape_chrdev_open(struct inode *inode, struct file *filp)
{
	...
	if (test_and_set_bit(IDE_DFLAG_BUSY, &drive->dev_flags)) {
		retval = -EBUSY;
		goto out_put_tape;
	}
	...
	if (retval) {
		clear_bit(IDE_DFLAG_BUSY, &drive->dev_flags);
		printk(KERN_ERR "ide-tape: %s: drive not ready\n", tape->name);
		goto out_put_tape;
	}

	idetape_read_position(drive);
	if (!test_bit(IDE_DFLAG_ADDRESS_VALID, &drive->dev_flags))
		(void)idetape_rewind_tape(drive);
...
static int idetape_chrdev_release(struct inode *inode, struct file *filp)
{
	...
	if (minor < 128 && test_bit(IDE_DFLAG_MEDIUM_PRESENT, &drive->dev_flags))
		(void) idetape_rewind_tape(drive);
	if (tape->chrdev_dir == IDETAPE_DIR_NONE) {
