...
struct inode *ubifs_iget(struct super_block *sb, unsigned long inum)
{
	...
	inode->i_atime.tv_sec  = (int32_t)le32_to_cpu(ino->atime.sec);
	inode->i_atime.tv_nsec = le32_to_cpu(ino->atime.nsec);
	inode->i_mtime.tv_sec  = (int32_t)le32_to_cpu(ino->mtime.sec);
	inode->i_mtime.tv_nsec = le32_to_cpu(ino->mtime.nsec);
	inode->i_ctime.tv_sec  = (int32_t)le32_to_cpu(ino->ctime.sec);
