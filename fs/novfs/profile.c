...
static int LocalPrint(char *Fmt, ...) __attribute__ ((__no_instrument_function__));
int DbgPrint(char *Fmt, ...) __attribute__ ((__no_instrument_function__));

static void doline(unsigned char *b, unsigned char *p, unsigned char *l)
...
static void profile_dump_memorylist(void *pf);
...
static ssize_t profile_inode_read(struct file *file, char *buf, size_t len,
			   loff_t * off);
static ssize_t profile_dentry_read(struct file *file, char *buf, size_t len,
			    loff_t * off);
static ssize_t profile_memory_read(struct file *file, char *buf, size_t len,
			    loff_t * off);
