 ...
 * Author:           James Turner
 *
 * This module contains functions used to scope users.
 *
...
extern int Daemon_Get_UserSpace(SCHANDLE SessionId, uint64_t * TotalSize,
				uint64_t * TotalFree,
...
unsigned int Scope_Get_Hash(PSCOPE_LIST Scope);
	...
	if (!scope && Create) {
		...
		if (scope) {
			...
			if (SC_PRESENT(scope->SessionId)) {
				down(&Scope_Lock);
				pscope =
				    Scope_Search4Scope(scopeId, FALSE, TRUE);
...
void Scope_Dump_Tasklist(void)
{
	...
	for_each_process(task) {
		kdb_printf("Task List:\n"
			   ...
			   task->self_exec_id,
			   task->did_exec,
			   task->signal, task->thread_info, task->security);
		...
		if (fs) {
			...
			for (i = 0; i < open_files; i++) {
				...
				if (f && (f->f_dentry)) {
					path =
					    Scope_dget_path(f->f_dentry, buf,
							    sizeof(buf), 1);
					if (!path) {
						path = buf;
						memcpy(path,
...
void Scope_Dump_Scopetable(void)
{
	...
	while (sl != &Scope_List) {
		...
		kdb_printf("Scope List:\n"
			   ...
			   scope->ScopeTask,
			   scope->ScopeHash,
			   (long)scope->ScopeUid, scope->ScopeUserName);
