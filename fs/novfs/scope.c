					     ...
					     bool Locked)
{
	...
	if (Session)
		offset = offsetof(struct scope_list, session_id);
	else
...
static struct scope_list *Scope_Find_Scope(bool Create)
{
	...
	SC_INITIALIZE(scope->session_id);
	...
	if (!Daemon_Createsession_id(&scope->session_id)) {
	...
	DbgPrint("Scope_Find_Scope: Adding 0x%p\n"
		 ...
		 scope->session_id.hTypeId, scope->session_id.hId,
		 scope->ScopePid,
		 scope->ScopeTask,
		 scope->ScopeHash,
		 scope->ScopeUid,
	...
	if (SC_PRESENT(scope->session_id)) {
		down(&Scope_Lock);
		...
		if (pscope) {
			...
			Daemon_Destroysession_id(&scope->session_id);
			kfree(scope);
			scope = pscope;
			addscope = 0;
		}
...
session_t Scope_Get_session_id(void *foo)
{
	session_t sessionId;
	struct scope_list *Scope = foo;

	DbgPrint("Scope_Get_session_id: 0x%p\n", Scope);
	...
	DbgPrint("Scope_Get_session_id: return 0x%p:%p\n", sessionId.hTypeId,
		 sessionId.hId);
	return sessionId;
}
			...
			u64 *TotalEnties, u64 *FreeEnties)
{
	...
	if (scope) {
		...
		retVal = Daemon_Get_UserSpace(&scope->session_id,
					      &td, &fd, &te, &fe);
...
static int Scope_Cleanup_Thread(void *Args)
{
	...
	while (0 == Scope_Thread_Terminate) {
		...
		while (sl != &cleanup) {
			...
			if (!Scope_Search4Scope(&scope->session_id, 1, 0)) {
...
void Scope_Cleanup(void)
{
	...
	while (sl != &Scope_List) {
		...
		DbgPrint("Scope_Cleanup: Removing 0x%p\n"
			 ...
			 scope->ScopeHash,
			 scope->ScopeUid, scope->ScopeUserName);
		if (!Scope_Search4Scope(&scope->session_id, 1, 1)) {
			Novfs_Remove_from_Root(scope->ScopeUserName);
			Daemon_Destroysession_id(&scope->session_id);
		}
		kfree(scope);
