...
extern int do_login(NclString * Server, NclString * Username,
		    NclString * Password, u_int * lgnId, session_t Session);

void GetUserData(NwcScanConnInfo * connInfo, PXPLAT_CALL_REQUEST cmd,
		 PXPLAT_CALL_REPLY reply);
void GetConnData(NwcGetConnInfo * connInfo, PXPLAT_CALL_REQUEST cmd,
		 PXPLAT_CALL_REPLY reply);
 ...
 *========================================================================*/
{
	...
	int retCode = 0;
	u_long cmdlen, datalen, replylen, cpylen;
	char *data;

	cpylen = copy_from_user(&ocbn, pdata->reqData, sizeof(ocbn));
	datalen =
	    sizeof(*openConn) + strlen_user(ocbn.pName->pString) +
	    strlen_user(ocbn.pServiceType);
	...
	if (cmd) {
		...
		if (reply) {
			...
			if (!retCode) {
				...
				connReply = (PNwdCOpenConnByName) reply->data;
				ocbn.RetConnHandle =
				    HandletoUint32(connReply->newConnHandle);
				*Handle = connReply->newConnHandle;

				cpylen =
				    copy_to_user(pdata->reqData, &ocbn,
						 sizeof(ocbn));
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		openConn->TranAddr.oAddress = *(u_int *) (&addr[2]);
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		openConn->uConnReference =
		    (HANDLE) (u_long) ocbr.uConnReference;
 ...
 *========================================================================*/
{
	...
	PXPLAT_CALL_REPLY reply;
	int retCode = -ENOMEM;
	u_long cmdlen, datalen, replylen, cpylen, totalLen;
	u_int x;
	PNwdCNCPReq ncpData;
	PNwdCNCPRep ncpReply;
	u_char *reqData;
	u_long actualReplyLength = 0;
	 ...
	 */
	frag =
	    Novfs_Malloc(xRequest.uNumReplyFrags * sizeof(NwcFrag), GFP_KERNEL);
	...
	cpylen =
	    copy_from_user(frag, xRequest.pReplyFrags,
			   xRequest.uNumReplyFrags * sizeof(NwcFrag));
	totalLen = 0;
	...
	datalen = 0;
	reqFrag =
	    Novfs_Malloc(xRequest.uNumRequestFrags * sizeof(NwcFrag),
			 GFP_KERNEL);
	if (!reqFrag) {
		Novfs_Free(frag);
		return (retCode);
	...
	cpylen =
	    copy_from_user(reqFrag, xRequest.pRequestFrags,
			   xRequest.uNumRequestFrags * sizeof(NwcFrag));
	cFrag = reqFrag;
	...
	if (cmd) {
		...
		ncpData->ConnHandle = (HANDLE) (u_long) xRequest.ConnHandle;
		...
		if (reply) {
			...
			for (x = 0; x < xRequest.uNumReplyFrags; x++) {
				...
				datalen =
				    min((u_long) cFrag->uLength, totalLen);
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		nwdClose = (PNwdCCloseConn) cmd->data;
		cmd->dataLen = sizeof(*nwdClose);
		nwdClose->ConnHandle = (HANDLE) (u_long) cc.ConnHandle;
		*Handle = (u_long) cc.ConnHandle;
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		pDauth->ConnHandle = (HANDLE) (u_long) pauth.ConnHandle;
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		cmd->dataLen = datalen;
		pDLisc->ConnHandle = (HANDLE) (u_long) lisc.ConnHandle;
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		pUconn->ConnHandle = (HANDLE) (u_long) ulc.ConnHandle;
		retCode =
		    Queue_Daemon_Command((void *)cmd, cmdlen, NULL, 0,
					 (void **)&reply, &replylen,
					 INTERRUPTIBLE);
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		pDAuth->ConnHandle = (HANDLE) (u_long) auth.ConnHandle;
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		pDConnInfo->ConnHandle = (HANDLE) (u_long) connInfo.ConnHandle;
 ...
 *========================================================================*/
{
	...
	case NWC_CONN_INFO_TRAN_ADDR:
		{
			u_char *dstData = connInfo->pReturnConnInfo;
			NwcTranAddr tranAddr;
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		pConn->ConnHandle = (HANDLE) (u_long) xplatCall.ConnHandle;
		status =
		    Queue_Daemon_Command((void *)cmd, cmdlen, NULL, 0,
					 (void **)&reply, &replylen,
					 INTERRUPTIBLE);
 ...
 *========================================================================*/
{
	...
	if (cmd) {
		...
		dmsg->uConnReference = (HANDLE) (u_long) msg.uConnReference;
		...
		if (reply) {
			...
			if (!status) {
				...
				cpylen =
				    (int)&((PNwcGetBroadcastNotification) 0)->
				    message;
				cp += cpylen;
				cpylen =
...
int NwdSetKeyValue(PXPLAT pdata, session_t Session)
{
	...
	if (cmd) {
		...
		pNewKey->ConnHandle = (HANDLE) (u_long) xplatCall.ConnHandle;
		str = (char *)pNewKey;
