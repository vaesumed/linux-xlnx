...
ssize_t Daemon_Receive_Reply(struct file *file, const char *buf, size_t nbytes, loff_t * ppos)
{
	daemon_command_t *que;
	size_t retValue = 0;
	void *reply;
		...
		case IOC_XPLAT:
			{
				...
				case NWC_SYS_CLOSE_CONN:
					...
					retCode = NwSysConnClose(&data, (u_long *)&handle, dh->session);
