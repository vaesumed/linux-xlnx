...
extern int Novfs_Read_Stream(HANDLE ConnHandle, u_char * Handle,
			     u_char * Buffer, size_t * Bytes, loff_t * Offset,
			     int User, session_t SessionId);
extern int Novfs_Write_Stream(HANDLE ConnHandle, u_char * Handle,
			      u_char * Buffer, size_t * Bytes, loff_t * Offset,
			      session_t SessionId);
extern int Novfs_Close_Stream(HANDLE ConnHandle, u_char * Handle,
			      session_t SessionId);
...
extern int NwSysConnClose(PXPLAT pdata, HANDLE * Handle, session_t Session);
...
/*===[ Function prototypes ]==============================================*/
int Daemon_Added_Resource(daemon_handle_t * DHandle, int Type, HANDLE CHandle,
			  u_char * FHandle, u_long Mode, u_long Size);
...
int Daemon_Get_UserSpace(SCHANDLE SessionId, uint64_t * TotalSize,
			 uint64_t * TotalFree, uint64_t * TotalDirectoryEnties,
			 uint64_t * FreeDirectoryEnties);
int Daemon_Library_close(struct inode *inode, struct file *file);
int Daemon_Library_ioctl(struct inode *inode, struct file *file,
			 unsigned int cmd, u_long arg);
int Daemon_Library_open(struct inode *inode, struct file *file);
ssize_t Daemon_Library_read(struct file *file, char *buf, size_t len,
			    loff_t * off);
ssize_t Daemon_Library_write(struct file *file, const char *buf, size_t len,
			     loff_t * off);
...
ssize_t Daemon_Receive_Reply(struct file *file, char *buf, size_t nbytes,
			     loff_t * ppos);
int Daemon_Remove_Resource(daemon_handle_t * DHandle, int Type, HANDLE CHandle,
			   u_long FHandle);

ssize_t Daemon_Send_Command(struct file *file, char *buf, size_t len,
			    loff_t * off);
int Daemon_SetMountPoint(char *Path);
void Daemon_Timer(u_long data);
int Daemon_getpwuid(uid_t uid, int unamelen, char *uname);
int Daemon_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
		 u_long arg);
void Init_Daemon_Queue(void);
int Queue_Daemon_Command(void *request, u_long reqlen, void *data, int dlen,
			 void **reply, u_long * replen, int interruptible);
void Queue_get(daemon_command_t * que);
void Queue_put(daemon_command_t * que);
void Uninit_Daemon_Queue(void);
int do_login(NclString * Server, NclString * Username, NclString * Password,
	     HANDLE * lgnId, session_t Session);
 ...
 *========================================================================*/
{
	...
	if (atomic_read(&Daemon_Open_Count)) {
		...
		que =
		    (daemon_command_t *) Novfs_Malloc(sizeof(*que), GFP_KERNEL);
		...
		if (que) {
			...
			que->timer.expires =
			    jiffies + (HZ * Daemon_Command_Timeout);
...
/*++======================================================================*/
ssize_t
Daemon_Receive_Reply(struct file * file, char *buf, size_t nbytes,
		     loff_t * ppos)
/*
 *
 *  Arguments:
 ...
 *========================================================================*/
{
	...
	if (reply) {
		if (reply->Reply.ErrorCode) {
			...
			replylen =
			    replylen - (int)(&((PGETPWUID_REPLY) 0)->UserName);
			if (replylen) {
 ...
 *========================================================================*/
{
	...
	if (reply) {
		if (reply->Reply.ErrorCode) {
			...
			retVal =
			    replylen -
			    (int)(&((PGET_VERSION_REPLY) 0)->Version);
			if (retVal < length) {
 ...
 *========================================================================*/
{
	...
	if (!copy_from_user(&lLogin, Login, sizeof(lLogin))) {
		if ((server.buffer =
		     Novfs_Malloc(lLogin.Server.length, GFP_KERNEL))) {
			...
			if (!copy_from_user
			    ((void *)server.buffer, lLogin.Server.data,
			     server.len)) {
				if ((username.buffer =
				     Novfs_Malloc(lLogin.UserName.length,
						  GFP_KERNEL))) {
					...
					if (!copy_from_user
					    ((void *)username.buffer,
					     ...
					     username.len)) {
						if ((password.buffer =
						     Novfs_Malloc(lLogin.Password.length,
								  GFP_KERNEL)))
						{
							...
							memset(password.buffer,
							       0, password.len);
							Novfs_Free(password.
								   buffer);
						}
					}
					memset(username.buffer, 0,
					       username.len);
...
int Daemon_Logout(PLOGOUT Logout, session_t Session)
{
	...
	if (!copy_from_user(&lLogout, Logout, sizeof(lLogout))) {
		if ((server.name =
		     Novfs_Malloc(lLogout.Server.length, GFP_KERNEL))) {
...
int Daemon_Library_ioctl(struct inode *inode, struct file *file, unsigned int cmd, u_long arg)
{
	...
	if (dh) {
		...
		case IOC_LOGIN:
			{
				retCode =
				    Daemon_Login((PLOGIN) arg, dh->session);
				break;
			}
		...
		case IOC_LOGOUT:
			{
				retCode =
				    Daemon_Logout((PLOGOUT) arg, dh->session);
				break;
			}
		...
		case IOC_XPLAT:
			{
				...
				case NWC_SYS_CLOSE_CONN:
					DbgPrint
					    ("[VFS XPLAT] Call NwSysCloseConn\n");
					retCode =
					    NwSysConnClose(&data, &handle,
							   dh->session);
				...
				case NWC_MAP_DRIVE:
					...
					retCode =
					    NwdSetMapDrive(&data, dh->session);
					break;
...
/*++======================================================================*/
unsigned int Daemon_Poll(struct file *file,
...
void Daemon_Dumpque(void)
{
	...
	while (que && (que != (daemon_command_t *) & Daemon_Queue.list.next)) {
		kdb_printf("DaemonQue:\n"
			   ...
			   que->request,
			   que->reqlen,
			   que->data, que->datalen, que->reply, que->replen);
		que = (daemon_command_t *) que->list.next;
	}
#endif
}
 ...
 *========================================================================*/
{
	...
	cpylen =
	    copy_from_user(&nh, pdata->reqData,
			   sizeof(NwcConvertNetWareHandle));
 ...
 *========================================================================*/
{
	...
	if (!retVal) {
		...
		drivemap =
		    Novfs_Malloc(sizeof(drive_map_t) + symInfo.linkOffsetLength,
				 GFP_KERNEL);
		if (drivemap) {
