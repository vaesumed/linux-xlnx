...
extern ssize_t
Daemon_Receive_Reply(struct file *file, const char *buf, size_t nbytes,
		     loff_t * ppos);

extern ssize_t
Daemon_Send_Command(struct file *file, char *buf, size_t len, loff_t * off);

extern int Daemon_ioctl(struct inode *inode, struct file *file,
			unsigned int cmd, unsigned long arg);

extern int Daemon_Library_close(struct inode *inode, struct file *file);
extern int Daemon_Library_ioctl(struct inode *inode, struct file *file,
				unsigned int cmd, unsigned long arg);
extern int Daemon_Library_open(struct inode *inode, struct file *file);
extern ssize_t Daemon_Library_read(struct file *file, char *buf, size_t len,
				   loff_t * off);
extern ssize_t Daemon_Library_write(struct file *file, const char *buf,
				    size_t len, loff_t * off);
extern loff_t Daemon_Library_llseek(struct file *file, loff_t offset,
				    int origin);
...
/*===[ Function prototypes ]==============================================*/
ssize_t
Novfs_User_proc_read(struct file *file, char *buf, size_t nbytes,
		     loff_t * ppos);

ssize_t
Novfs_User_proc_write(struct file *file, char *buf, size_t nbytes,
		      loff_t * ppos);

int Novfs_User_proc_ioctl(struct inode *inode, struct file *file,
			  unsigned int cmd, unsigned long arg);
...
/*===[ Global variables ]=================================================*/
struct proc_dir_entry *Novfs_Procfs_dir, *Novfs_Control, *Novfs_Library,
    *Novfs_Version;
...
int Novfs_Get_Version(char *page, char **start, off_t off, int count, int *eof,
		      void *data)
{
	char *buf, tbuf[48];
	int len = 0, i;
...
int Init_Procfs_Interface(void)
{
	...
	if (Novfs_Procfs_dir) {
		...
		Novfs_Control =
		    create_proc_entry("Control", 0600, Novfs_Procfs_dir);
		...
		Novfs_Library =
		    create_proc_entry("Library", 0666, Novfs_Procfs_dir);
		if (Novfs_Library) {
