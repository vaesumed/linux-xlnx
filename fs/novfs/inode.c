...
static ssize_t Novfs_dir_read(struct file *file, char __user *buf, size_t len, loff_t * off);
...
static int Novfs_dir_fsync(struct file *file, struct dentry *dentry, int datasync);

/*
 * Declared address space operations
 */
int Novfs_a_writepage(struct page *page, struct writeback_control *wbc);
static int Novfs_a_writepages(struct address_space *mapping,
 ...
 */
static ssize_t Novfs_f_read(struct file *, char __user *, size_t, loff_t *);
static ssize_t Novfs_f_write(struct file *, const char __user *, size_t, loff_t *);
...
static struct dentry_operations Novfs_dentry_operations = {
...
static struct file_operations Novfs_dir_operations = {
	.owner = THIS_MODULE,
	.open = Novfs_dir_open,
...
static struct backing_dev_info Novfs_backing_dev_info = {
...
static DECLARE_MUTEX(InodeList_lock);
static LIST_HEAD(InodeList);
static DECLARE_MUTEX(TimeDir_Lock);
static uint64_t inHAXTime;
static int inHAX;

static unsigned long InodeCount = 0;
static unsigned long DCCount = 0;
	...
	if (dentry && dentry->d_parent &&
		...
		if (info) {
			...
			if (IS_ROOT(dentry->d_parent)) {
				session = Scope_Get_session_id(Scope_Get_ScopefromName(&dentry->d_name));
			} else {
				session = Scope_Get_session_id(id->Scope);
			}
...
static ssize_t Novfs_dir_read(struct file *file, char __user *buf,
			      size_t len, loff_t * off)
{
	DbgPrint("%s: %lld %d Name %.*s\n", __func__, *off, len,
		 file->f_dentry->d_name.len, file->f_dentry->d_name.name);
...
static void Novfs_Dump_Info(struct entry_info *info)
{
	...
	DbgPrint("%s: mtime = %s\n", __func__, mtime_buf);
	DbgPrint("%s: ctime = %s\n", __func__, ctime_buf);
	DbgPrint("%s: namelength = %i\n", __func__, info->namelength);
	DbgPrint("%s: name = %s\n", __func__, namebuf);
}

/*++======================================================================*/
static void processList(struct file *file, void *dirent, filldir_t filldir,
			char *list, int type, session_t session_id)
{
...
static int processEntries(struct file *file, void *dirent, filldir_t filldir,
...
// Use this hack by default
	...
	if (inode && inode->FSPRIVATE) {
		sessionId =
		    Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)-> Scope);
...
static int Novfs_dir_fsync(struct file *file, struct dentry *dentry, int datasync)
{
	DbgPrint("%s: Name %.*s\n", __func__, file->f_dentry->d_name.len,
		 file->f_dentry->d_name.name);
	return simple_sync_file(file, dentry, datasync);
}

static ssize_t Novfs_f_read(struct file * file, char __user *buf,
			    size_t len, loff_t *off)
{
...
static ssize_t Novfs_f_write(struct file *file, const char __user *buf,
			     size_t len, loff_t *off)
{
	size_t thiswrite;
	...
	if (file->f_dentry &&
		...
		DbgPrint("%s: (0x%p 0x%p 0x%p %d %lld %.*s)\n", __func__,
		...
		if (PageCache &&
			...
			if (file->f_flags & O_APPEND) {
				...
				DbgPrint("%s: appending to end %lld %.*s\n", __func__,
...
static int Novfs_flush_mapping(HANDLE Handle, struct address_space *mapping,
			       session_t Session)
{
 ...
 *========================================================================*/
{
	...
	session = Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)->Scope);
			      ...
			      struct writeback_control *wbc)
{
	...
	DbgPrint("%s: inode=0x%p mapping=0x%p wbc=0x%p nr_to_write=%d\n",
		  __func__, inode, mapping, wbc, wbc->nr_to_write);
	...
	if (id)
		DbgPrint("%s: inode=0x%p fh=0x%p dlist=0x%p pages=0x%p %s\n",
			 __func__, inode, fh, dlist, pages, id->Name);
	else
		DbgPrint("%s: inode=0x%p fh=0x%p dlist=0x%p pages=0x%p\n",
			 __func__, inode, fh, dlist, pages);
	...
	if (dlist && pages) {
		...
		while (!done && (wbc->nr_to_write > 0)) {
			...
			for (; i < nr_pages; i++) {
				...
				DbgPrint("%s: pos=0x%llx index=%d page->index=%d next_index=%d\n",
					 __func__, pos, index, page->index, next_index);
				...
				dlist_idx++;
				DbgPrint("%s: Add page=0x%p index=0x%lx\n", __func__, page, page->index);
			}

			DbgPrint("%s: dlist_idx=%d\n", __func__, dlist_idx);
			if (dlist_idx) {
				...
				do {
					...
					page_cache_release((struct page *)
							   dlptr[dlist_idx -
								 1].page);
					DbgPrint("%s: release page=0x%p index=0x%lx\n",
						 __func__, dlptr[dlist_idx - 1].page,
						 ((struct page *)dlptr[dlist_idx - 1].page)->index);
					if (!retCode) {
						wbc->nr_to_write--;
					}
				} while (--dlist_idx);
 ...
 *========================================================================*/
{
	...
	if (inode) {
		...
		if (inode->FSPRIVATE) {
			session =
			    Scope_Get_session_id(((struct inode_data *)inode->
						 FSPRIVATE)->Scope);
			if (0 == SC_PRESENT(session)) {
				((struct inode_data *)inode->FSPRIVATE)->Scope =
				    Scope_Get_ScopefromPath(file->f_dentry);
 ...
 *========================================================================*/
{
	...
	if (mode | S_IFREG) {
		if (dir->FSPRIVATE) {
			session =
			    Scope_Get_session_id(((struct inode_data *)dir->FSPRIVATE)->
						Scope);
			if (0 == SC_PRESENT(session)) {
				((struct inode_data *) dir->FSPRIVATE)->Scope =
				    Scope_Get_ScopefromPath(dentry);
				session = Scope_Get_session_id(((struct inode_data *)dir->FSPRIVATE)->Scope);
			}
 ...
 *========================================================================*/
{
	...
	if (dir->FSPRIVATE) {
		session =
		    Scope_Get_session_id(((struct inode_data *)dir->FSPRIVATE)->Scope);
		if (0 == SC_PRESENT(session)) {
			((struct inode_data *)dir->FSPRIVATE)->Scope =
			    Scope_Get_ScopefromPath(dentry);
			session =
					...
					if (!Novfs_d_strcmp(&newname, &oldname)) {
						...
						if (od->d_inode
							...
							if ((nd->d_inode) &&
								...
								session = Scope_Get_session_id(((struct inode_data *)ndir->FSPRIVATE)->Scope);
								if (0 == SC_PRESENT(session)) {
 ...
 *========================================================================*/
{
	...
	if (inode && inode->FSPRIVATE) {
		sessionId =
		    Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)->Scope);
		DbgPrint("Novfs_i_getxattr: session_id = %u\n", sessionId);
		//if (0 == sessionId)
		if (0 == SC_PRESENT(sessionId)) {
			((struct inode_data *) inode->FSPRIVATE)->Scope = Scope_Get_ScopefromPath(dentry);
			sessionId = Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)->Scope);
			DbgPrint("Novfs_i_getxattr: session_id = %u\n",
				 sessionId);
		}
	}
 ...
 *========================================================================*/
{
	...
	if (inode && inode->FSPRIVATE) {
		sessionId =
		    Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)->
					Scope);
		DbgPrint("Novfs_i_setxattr: session_id = %u\n", sessionId);
		//if (0 == sessionId)
		if (0 == SC_PRESENT(sessionId)) {
			((struct inode_data *)inode->FSPRIVATE)->Scope =
			    Scope_Get_ScopefromPath(dentry);
...
int Novfs_i_listxattr(struct dentry *dentry, char *buffer, size_t buffer_size)
{
	...
	if (inode && inode->FSPRIVATE) {
		sessionId =
		    Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)-> Scope);
		DbgPrint("Novfs_i_listxattr: session_id = %u\n", sessionId);
		//if (0 == sessionId)
		if (0 == SC_PRESENT(sessionId)) {
			((struct inode_data *)inode->FSPRIVATE)->Scope = Scope_Get_ScopefromPath(dentry);
			sessionId = Scope_Get_session_id(((struct inode_data *)inode->FSPRIVATE)->Scope);
