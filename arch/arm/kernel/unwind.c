...
static struct unwind_idx *unwind_find_idx(unsigned long addr)
{
	...
	if (core_kernel_text(addr))
		/* main unwind table */
		idx = search_index(addr, __start_unwind_idx, __stop_unwind_idx - 1);
	else {
		...
		list_for_each_entry(table, &unwind_tables, list) {
			if (addr >= table->begin_addr && addr < table->end_addr) {
				idx = search_index(addr, table->start, table->stop - 1);
				break;
			}
		}
...
static int unwind_exec_insn(struct unwind_ctrl_block *ctrl)
{
	...
	pr_debug("%s: fp = %08lx sp = %08lx lr = %08lx pc = %08lx\n",
		 __FUNCTION__, ctrl->vrs[FP], ctrl->vrs[SP], ctrl->vrs[LR], ctrl->vrs[PC]);
...
int unwind_frame(struct stackframe *frame)
{
	...
	pr_debug("%s(pc = %08lx lr = %08lx sp = %08lx)\n",
		 __FUNCTION__, frame->pc, frame->lr, frame->sp);
				      ...
				      unsigned long text_addr, unsigned long text_size)
{
	...
	pr_debug("%s(%08lx, %08lx, %08lx, %08lx)\n",
		 __FUNCTION__, start, size, text_addr, text_size);
